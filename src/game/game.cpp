...
void Game::update()
{
	for (auto w = weapons.begin(); w != weapons.end();) {
		if ((*w) != nullptr) {
			if (mainPlayer.getCharacter()
					.getBody()
					->getCollisionBox()
					.check((*w)->getCollisionBox()) &&
				mainPlayer.getCharacter().isPicking()) {
	...
	for (auto b = bullets.begin(); b != bullets.end();) {
		if ((*b) != nullptr) {
			if ((*b)->getPosition().x < 0 ||
				(*b)->getPosition().y < 0 ||
				(*b)->getPosition().x >
					BBOP_WINDOW_RESOLUTION.x ||
				(*b)->getPosition().y >
					BBOP_WINDOW_RESOLUTION.y) {
				bullets.erase(b);
			} else {
				for (auto &dummy : dummys) {
					if (dummy->getBody()->getCollisionBox().check(
						    (*b)->getCollisionBox()))
						dummy->switchState(
							BodyState::dead);
				}
	...
	if (musicChannel == -1 && TOGGLE_MUSIC) {
		musicChannel = Mix_PlayChannel(-1,
			RessourceManager::getSound(
				"assets/sounds/musics/05.wav"),
			-1);
	...
	for (auto &d : dummys) {
		if (d) {
			...
			bbopDebugCollisionBox(
				d->getBody()->getCollisionBox(), scene);
		}
	}
...
Camera *Game::getMainPlayerCam() { return &mainPlayerCam; }

void Game::addBullet(Bullet *b) { bullets.push_back(b); }
