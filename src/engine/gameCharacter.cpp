    ...
    : legs("assets/default.png"), body("assets/default.png")
{
	...
	// TODO - Patcher les collision de Bbop (checkRotation et gestion de l'offset
	// avec l'origine)
	...
	bodyAnimations[{BodyState::idle, WeaponName::fist}].textures =
		bbopLoadSpriteSheet(path.c_str(), 1, 1);

	path = "assets/" + characterFolder + "body/running.png";
	bodyAnimations[{BodyState::running, WeaponName::fist}].textures =
		bbopLoadSpriteSheet(path.c_str(), 8, 1);

	path = "assets/" + characterFolder + "body/attacking.png";
	bodyAnimations[{BodyState::attacking, WeaponName::fist}].textures =
		bbopLoadSpriteSheet(path.c_str(), 6, 1);
	bodyAnimations[{BodyState::attacking, WeaponName::fist}]
		.knockoutRange = {1, 4};

	path = "assets/" + characterFolder + "body/idle_bate.png";
	bodyAnimations[{BodyState::idle, WeaponName::bate}].textures =
		bbopLoadSpriteSheet(path.c_str(), 1, 1);

	path = "assets/" + characterFolder + "body/running_bate.png";
	bodyAnimations[{BodyState::running, WeaponName::bate}].textures =
		bbopLoadSpriteSheet(path.c_str(), 8, 1);

	path = "assets/" + characterFolder + "body/attacking_bate.png";
	bodyAnimations[{BodyState::attacking, WeaponName::bate}].textures =
		bbopLoadSpriteSheet(path.c_str(), 1, 8);
	bodyAnimations[{BodyState::attacking, WeaponName::bate}].letalRange = {
		3, 7};

	path = "assets/" + characterFolder + "body/idle_gun.png";
	bodyAnimations[{BodyState::idle, WeaponName::gun}].textures =
		bbopLoadSpriteSheet(path.c_str(), 1, 1);

	path = "assets/" + characterFolder + "body/running_gun.png";
	bodyAnimations[{BodyState::running, WeaponName::gun}].textures =
		bbopLoadSpriteSheet(path.c_str(), 8, 1);

	path = "assets/" + characterFolder + "body/attacking_gun.png";
	bodyAnimations[{BodyState::attacking, WeaponName::gun}].textures =
		bbopLoadSpriteSheet(path.c_str(), 1, 2);

	// nombre aléatoire pour le sprite de mort
	std::string deadId = std::to_string(
		std::uniform_int_distribution<int>(0, 2)(RANDOM_ENGINE));
	path = "assets/" + characterFolder + "body/dead" + deadId + ".png";
	bodyAnimations[{BodyState::dead, WeaponName::fist}].textures =
		bbopLoadSpriteSheet(path.c_str(), 1, 1);

	path = "assets/" + characterFolder + "legs/" +
		bodyStateToString(BodyState::idle) + ".png";
	legsAnimations[LegsState::idle].textures =
		bbopLoadSpriteSheet(path.c_str(), 1, 1);

	path = "assets/" + characterFolder + "legs/" +
		bodyStateToString(BodyState::running) + ".png";
	legsAnimations[LegsState::running].textures =
		bbopLoadSpriteSheet(path.c_str(), 12, 1);
}
...
void GameCharacter::update()
{
	...
	speed = (speed +
		(movement * acceleration * static_cast<float>(DELTA_TIME)));

	// gestion du déplacement
	setPosition(getPosition().x + speed.x * DELTA_TIME,
		getPosition().y + speed.y * DELTA_TIME);
	// fais suivre les legs et le body sur la posisition du character
	legs.setPosition(getPosition());
	body.setPosition(getPosition());
	if (weapon == nullptr)
		std::cerr << "Erreur avec le pointeur weapon de GameCharacter"
			  << std::endl;
	...
	if (movement == glm::vec2(0, 0)) {
		...
		if (walkAudioCanal == -1) {
			walkAudioCanal = Mix_PlayChannel(-1,
				RessourceManager::getSound(
	...
	if (glfwGetTime() - bodyAnimations[bState].lastFrameTime >=
		ANIM_SPEED / GAME_SPEED) {
		bodyAnimations[bState].frame++;
		bodyAnimations[bState].lastFrameTime = glfwGetTime();

		if (bodyAnimations[bState].frame >=
			bodyAnimations[bState].textures.size()) {
	...
	if (glfwGetTime() - legsAnimations[lState].lastFrameTime >=
		ANIM_SPEED / GAME_SPEED) {
		legsAnimations[lState].frame++;
		legsAnimations[lState].lastFrameTime = glfwGetTime();

		if (legsAnimations[lState].frame ==
			legsAnimations[lState].textures.size())
			legsAnimations[lState].frame = 0;
	}

	legs.setTexture(
		legsAnimations[lState].textures[legsAnimations[lState].frame]);
	body.setTexture(
		bodyAnimations[bState].textures[bodyAnimations[bState].frame]);
	...
	ImGui::Text("BodyState pair: {%s, %s}",
		bodyStateToString(bState.first).c_str(),
		weaponNameToString(bState.second).c_str());
...
void GameCharacter::switchState(BodyState state)
{
	...
	case BodyState::idle:
		...
		if (bState.first == BodyState::attacking &&
			dynamic_cast<Firearme *>(weapon) == nullptr)
	...
	case BodyState::attacking: {
		...
		if (a != nullptr) {
			if (a->canFire()) {
				...
				bodyAnimations[bState].lastFrameTime =
					glfwGetTime();
			}
			break;
		}
...
void GameCharacter::lookAt(glm::vec2 point) { lookingPoint = point; }
...
void GameCharacter::move(float x, float y) { this->move(glm::vec2(x, y)); }

void GameCharacter::useWeapon() { this->switchState(BodyState::attacking); }

Sprite *GameCharacter::getBody() { return &body; }

Sprite *GameCharacter::getLegs() { return &legs; }

bool GameCharacter::isLetal()
{
	if (bState.first == BodyState::attacking &&
		bodyAnimations[bState].frame >=
			bodyAnimations[bState].letalRange.first &&
		bodyAnimations[bState].frame <=
			bodyAnimations[bState].letalRange.second)
		return true;
	return false;
}

void GameCharacter::pickup(bool s) { pickupFlag = s; }

bool GameCharacter::isPicking() { return pickupFlag; }
