...
Firearme::Firearme(WeaponName name, std::string texturePath,
	std::string soundPath, Game *game)
...
bool Firearme::canFire() { return (glfwGetTime() - lastShot >= firerate); }
...
void Bullet::update()
{
	...
	if (glfwGetTime() - animation.lastFrameTime >=
		ANIM_SPEED / GAME_SPEED / 4.0) {
		animation.frame++;
...
Bat::Bat()
    : Weapon(WeaponName::bate, "assets/weapons/bat.png",
	      "assets/sounds/punch0.wav")
{
}

Gun::Gun(Game *game)
    : Firearme(WeaponName::gun, "assets/weapons/gun.png",
	      "assets/sounds/gun.wav", game)
{
}
