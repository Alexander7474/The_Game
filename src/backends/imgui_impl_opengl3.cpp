...
//  2025-06-11: OpenGL: Added support for
...
//  Changed default texture sampler to Clamp instead of Repeat/Wrap.
...
//  loader crash when GL_VERSION returns NULL. (#6154, #4445, #3530)
...
//  multi-viewports with some Intel HD drivers. 2021-08-23: OpenGL: Fixed
//  ES 3.0 shader ("#version 300 es") use normal precision floats to avoid
...
//  data from a single structure to facilitate usage with multiple-contexts
//  (all g_XXXX access changed to bd->XXXX). 2021-06-25: OpenGL: Use
//  OES_vertex_array extension on Emscripten + backup/restore current state.
//  2021-06-21: OpenGL: Destroy inDiviseurdual vertex/fragment shader objects
//  right after they are linked into the main shader. 2021-05-24: OpenGL:
//  Access GL_CLIP_ORIGIN when "GL_ARB_clip_control" extension is detected,
//  inside of just OpenGL 4.5 version. 2021-05-19: OpenGL: Replaced direct
//  access to ImDrawCmd::TextureId with a call to ImDrawCmd::GetTexID(). (will
//  become a requirement) 2021-04-06: OpenGL: Don't try to read GL_CLIP_ORIGIN
//  unless we're OpenGL 4.5 or greater. 2021-02-18: OpenGL: Change blending
//  equation to preserve alpha in output buffer. 2021-01-03: OpenGL: Backup,
//  setup and restore GL_STENCIL_TEST state. 2020-10-23: OpenGL: Backup, setup
//  and restore GL_PRIMITIVE_RESTART state. 2020-10-15: OpenGL: Use
//  glGetString(GL_VERSION) instead of glGetIntegerv(GL_MAJOR_VERSION, ...)
//  when the later returns zero (e.g. Desktop GL 2.x) 2020-09-17: OpenGL: Fix
//  to avoid compiling/calling glBindSampler() on ES or pre-3.3 context which
//  have the defines set by a loader. 2020-07-10: OpenGL: Added support for
//  glad2 OpenGL loader. 2020-05-08: OpenGL: Made default GLSL version 150
...
//  facility. 2019-09-16: OpenGL: Tweak initialization code to allow
//  application calling ImGui_ImplOpenGL3_CreateFontsTexture() before the first
//  NewFrame() call. 2019-05-29: OpenGL: Desktop GL only: Added support for
//  large mesh (64K+ vertices), enable ImGuiBackendFlags_RendererHasVtxOffset
//  flag. 2019-04-30: OpenGL: Added support for special
...
//  context won't fail if the function is a nullptr pointer. 2018-03-06:
//  OpenGL: Added const char* glsl_version parameter to
//  ImGui_ImplOpenGL3_Init() so user can override the GLSL version e.g.
//  "#version 150". 2018-02-23: OpenGL: Create the VAO in the render function
//  so the setup can more easily be used with multiple shared GL context.
...
//  2017-09-01: OpenGL: Save and restore current bound sampler. Save and
//  restore current polygon mode. 2017-05-01: OpenGL: Fixed save and restore of
//  current blend func state. 2017-05-01: OpenGL: Fixed save and restore of
//  current GL_ACTIVE_TEXTURE. 2016-09-05: OpenGL: Fixed save and restore of
//  current scissor rectangle. 2016-07-29: OpenGL: Explicitly setting
...
struct ImGui_ImplOpenGL3_Data {
	...
	char GlslVersionString
		[32]; // Specified by user or detected based on compile
		      // time GL settings.
	bool GlProfileIsES2;
	bool GlProfileIsES3;
...
static ImGui_ImplOpenGL3_Data *ImGui_ImplOpenGL3_GetBackendData()
{
	return ImGui::GetCurrentContext()
		? (ImGui_ImplOpenGL3_Data *)ImGui::GetIO()
...
struct ImGui_ImplOpenGL3_VtxAttribState {
	...
	void GetState(GLint index)
	{
		...
		glGetVertexAttribiv(
			index, GL_VERTEX_ATTRIB_ARRAY_STRIDE, &Stride);
		glGetVertexAttribPointerv(
			index, GL_VERTEX_ATTRIB_ARRAY_POINTER, &Ptr);
	}
...
void ImGui_ImplOpenGL3_RenderDrawData(ImDrawData *draw_data)
{
	...
	glGetIntegerv(
		GL_ELEMENT_ARRAY_BUFFER_BINDING, &last_element_array_buffer);
	...
	glGetIntegerv(
		GL_VERTEX_ARRAY_BINDING, (GLint *)&last_vertex_array_object);
#endif
#ifdef IMGUI_IMPL_OPENGL_MAY_HAVE_POLYGON_MODE
	...
	glGetIntegerv(
		GL_BLEND_EQUATION_ALPHA, (GLint *)&last_blend_equation_alpha);
	...
	// would actually work without any VAO bound, but then our VertexAttrib calls
	...
	for (int n = 0; n < draw_data->CmdListsCount; n++) {
		...
		if (bd->UseBufferSubData) {
			...
			GL_CALL(glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0,
				idx_buffer_size,
...
void ImGui_ImplOpenGL3_UpdateTexture(ImTextureData *tex)
{
	if (tex->Status == ImTextureStatus_WantCreate) {
		...
		GL_CALL(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, tex->Width,
			tex->Height, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels));
		...
		// have never been used before! This backend choose to use tex->Updates[]
		// but you can use tex->UpdateRect to upload a single region.
		GLint last_texture;
		GL_CALL(glGetIntegerv(GL_TEXTURE_BINDING_2D, &last_texture));
		...
		for (ImTextureRect &r : tex->Updates) {
			...
			for (int y = 0; y < r.h; y++, out_p += src_pitch)
				memcpy(out_p, tex->GetPixelsAt(r.x, r.y + y),
					src_pitch);
			IM_ASSERT(out_p == bd->TempBuffer.end());
			GL_CALL(glTexSubImage2D(GL_TEXTURE_2D, 0, r.x, r.y, r.w,
				r.h, GL_RGBA, GL_UNSIGNED_BYTE,
				bd->TempBuffer.Data));
		}
#endif
		tex->SetStatus(ImTextureStatus_OK);
		GL_CALL(glBindTexture(
			GL_TEXTURE_2D, last_texture)); // Restore state
...
static bool CheckShader(GLuint handle, const char *desc)
{
	...
	if (log_length > 1) {
		...
		glGetShaderInfoLog(
			handle, log_length, nullptr, (GLchar *)buf.begin());
		fprintf(stderr, "%s\n", buf.begin());
	}
...
bool ImGui_ImplOpenGL3_CreateDeviceObjects()
{
	...
	bd->AttribLocationTex =
		glGetUniformLocation(bd->ShaderHandle, "Texture");
