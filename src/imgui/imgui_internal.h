...
#define IMGUI_DEBUG_LOG_FONT(...)                                              \
	do {                                                                   \
		...
		0) // Called from ImFontAtlas function which may operate without a \
		// context.
...
IMGUI_API ImGuiID ImHashData(
	const void *data, size_t data_size, ImGuiID seed = 0);
IMGUI_API ImGuiID ImHashStr(
	const char *data, size_t data_size = 0, ImGuiID seed = 0);
...
IMGUI_API void ImFormatStringToTempBuffer(const char **out_buf,
	const char **out_buf_end, const char *fmt, ...) IM_FMTARGS(3);
IMGUI_API void ImFormatStringToTempBufferV(const char **out_buf,
	const char **out_buf_end, const char *fmt, va_list args) IM_FMTLIST(3);
...
static inline ImVec2 ImClamp(
	const ImVec2 &v, const ImVec2 &mn, const ImVec2 &mx)
{
...
static inline float ImLinearRemapClamp(
...
inline bool ImTriangleIsClockwise(
...
// Helper: ImStableVector<>
// Allocating chunks of BLOCK_SIZE items. Objects pointers are never
// invalidated when growing, only by clear(). Important: does not destruct
// anything! Implemented only the minimum set of functions we need for it.
...
struct ImGuiTextIndex {
	...
	int EndOffset =
		0; // Because we don't own text buffer we need to maintain
...
#define IM_DRAWLIST_CIRCLE_AUTO_SEGMENT_CALC(_RAD, _MAXERROR)                  \
	ImClamp(IM_ROUNDUP_TO_EVEN((int)ImCeil(IM_PI /                         \
			ImAcos(1 - ImMin((_MAXERROR), (_RAD)) / (_RAD)))),     \
...
struct IMGUI_API ImDrawListSharedData {
	...
	ImDrawListFlags
		InitialFlags; // Initial flags at the beginning of the frame
	...
	ImGuiContext *Context; // [OPTIONAL] Link to Dear ImGui context. 99% of
		// ImDrawList/ImFontAtlas can function without an ImGui
		// context, but this facilitate handling one legacy edge case.

	// Lookup tables
	ImVec2 ArcFastVtx
		[IM_DRAWLIST_ARCFAST_TABLE_SIZE]; // Sample points on the
	...
	ImU8 CircleSegmentCounts
		[64]; // Precomputed segment count for given radius
...
struct ImGuiDataTypeStorage {
	ImU8 Data
		[8]; // Opaque storage to fit any data up to ImGuiDataType_COUNT
};
...
enum ImGuiItemFlagsPrivate_ {
	...
	ImGuiItemFlags_ReadOnly = 1
		<< 11, // false     // [ALPHA] Allow hovering interactions but
	// underlying value is not changed.
	ImGuiItemFlags_MixedValue = 1
		<< 12, // false     // [BETA] Represent a mixed/indeterminate value,
	...
	ImGuiItemFlags_NoNavDisableMouseHover = 1
		<< 15, // false     // Nav keyboard/gamepad mode doesn't disable hover
	// highlight (behave as if NavHighlightItemUnderNav==false).
	ImGuiItemFlags_NoMarkEdited = 1
		<< 16, // false     // Skip calling MarkItemEdited()
	ImGuiItemFlags_NoFocus = 1
	...
	ImGuiItemFlags_IsMultiSelect = 1
		<< 22, // false     // Set by SetNextItemSelectionUserData()
...
enum ImGuiItemStatusFlags_ {
	...
	ImGuiItemStatusFlags_HasShortcut = 1
		<< 10, // g.LastItemData.Shortcut valid. Set by SetNextItemShortcut() ->
		       // ItemAdd().
...
enum ImGuiHoveredFlagsPrivate_ {
	ImGuiHoveredFlags_DelayMask_ = ImGuiHoveredFlags_DelayNone |
		ImGuiHoveredFlags_DelayShort | ImGuiHoveredFlags_DelayNormal |
...
enum ImGuiInputTextFlagsPrivate_ {
	...
	ImGuiInputTextFlags_LocalizeDecimalPoint = 1
...
enum ImGuiButtonFlagsPrivate_ {
	...
	ImGuiButtonFlags_NoSetKeyOwner = 1
		<< 20, // don't set key/input owner on the initial click (note: mouse
	// buttons are keys! often, the key in question will be
	// ImGuiKey_MouseLeft!)
	ImGuiButtonFlags_NoTestKeyOwner = 1
		<< 21, // don't test key/input owner when polling the key (note: mouse
	// buttons are keys! often, the key in question will be
	// ImGuiKey_MouseLeft!)
...
enum ImGuiSelectableFlagsPrivate_ {
	...
	ImGuiSelectableFlags_NoSetKeyOwner = 1
		<< 27, // Don't set key/input owner on the initial click (note: mouse
		       // buttons are keys! often, the key in question will be
		       // ImGuiKey_MouseLeft!)
};
...
enum ImGuiTreeNodeFlagsPrivate_ {
	ImGuiTreeNodeFlags_NoNavFocus = 1
		<< 27, // Don't claim nav focus when interacting with this item (#8551)
	ImGuiTreeNodeFlags_ClipLabelForTrailingButton = 1
		<< 28, // FIXME-WIP: Hard-coded for CollapsingHeader()
	ImGuiTreeNodeFlags_UpsideDownArrow = 1
		<< 29, // FIXME-WIP: Turn Down arrow into an Up arrow, for reversed trees
	// (#6517)
	ImGuiTreeNodeFlags_OpenOnMask_ = ImGuiTreeNodeFlags_OpenOnDoubleClick |
...
enum ImGuiFocusRequestFlags_ {
	...
	ImGuiFocusRequestFlags_RestoreFocusedChild = 1
		<< 0, // Find last focused child (if any) and focus it instead.
	ImGuiFocusRequestFlags_UnlessBelowModal = 1
		<< 1, // Do not set focus if the window is below a modal.
};
...
enum ImGuiTooltipFlags_ {
	...
	ImGuiTooltipFlags_OverridePrevious = 1
...
enum ImGuiLogFlags_ {
	...
	ImGuiLogFlags_OutputMask_ = ImGuiLogFlags_OutputTTY |
...
struct IMGUI_API ImGuiMenuColumns {
	...
	ImU16 OffsetMark;
	ImU16 Widths
		[4]; // Width of:   Icon, Label, Shortcut, Mark  (accumulators
		     // for current frame)
...
struct IMGUI_API ImGuiInputTextState {
	...
	void OnKeyPressed(
		int key); // Cannot be inline because we call in code in
...
enum ImGuiWindowRefreshFlags_ {
	...
	ImGuiWindowRefreshFlags_TryToAvoidRefresh = 1
		<< 0, // [EXPERIMENTAL] Try to keep existing contents, USER MUST NOT
		      // HONOR BEGIN() RETURNING FALSE AND NOT APPEND.
	ImGuiWindowRefreshFlags_RefreshOnHover = 1
		<< 1, // [EXPERIMENTAL] Always refresh on hover
	ImGuiWindowRefreshFlags_RefreshOnFocus = 1
		<< 2, // [EXPERIMENTAL] Always refresh on focus
...
struct ImGuiPopupData {
	...
	ImGuiWindow
		*Window; // Resolved on BeginPopup() - may stay unresolved if
...
struct ImGuiKeyRoutingTable {
	ImGuiKeyRoutingIndex Index
		[ImGuiKey_NamedKey_COUNT]; // Index of first entry in Entries[]
...
enum ImGuiInputFlagsPrivate_ {
	...
	ImGuiInputFlags_RepeatUntilRelease = 1
	...
	ImGuiInputFlags_RepeatUntilKeyModsChange = 1
		<< 5, // Stop repeating when released OR if keyboard mods are changed
	...
	ImGuiInputFlags_RepeatUntilOtherKeyPress = 1
		<< 7, // Stop repeating when released OR if any other keyboard key is
	...
	ImGuiInputFlags_LockThisFrame = 1
		<< 20, // Further accesses to key data will require EXPLICIT owner ID
	...
	ImGuiInputFlags_CondHovered = 1
		<< 22, // Only set if item is hovered (default to both)
	ImGuiInputFlags_CondActive = 1
		<< 23, // Only set if item is active (default to both)
	...
	ImGuiInputFlags_CondMask_ =
		ImGuiInputFlags_CondHovered | ImGuiInputFlags_CondActive,
	ImGuiInputFlags_RouteTypeMask_ = ImGuiInputFlags_RouteActive |
...
enum ImGuiActivateFlags_ {
	...
	ImGuiActivateFlags_FromTabbing = 1
		<< 3, // Activation requested by a tabbing request
	ImGuiActivateFlags_FromShortcut = 1
		<< 4, // Activation requested by an item shortcut via
...
enum ImGuiNavRenderCursorFlags_ {
	...
	ImGuiNavRenderCursorFlags_Compact = 1
		<< 1, // Compact highlight, no padding/distance from focused item
	ImGuiNavRenderCursorFlags_AlwaysDraw = 1
		<< 2, // Draw rectangular highlight if (g.NavId == id) even when
...
enum ImGuiNavMoveFlags_ {
	...
	ImGuiNavMoveFlags_WrapMask_ = ImGuiNavMoveFlags_LoopX |
		ImGuiNavMoveFlags_LoopY | ImGuiNavMoveFlags_WrapX |
	...
	ImGuiNavMoveFlags_FocusApi = 1
		<< 9, // Requests from focus API can land/focus/activate items even if
	...
	ImGuiNavMoveFlags_NoSetNavCursorVisible = 1
		<< 14, // Do not alter the nav cursor visible state
	ImGuiNavMoveFlags_NoClearActiveId = 1
...
enum ImGuiTypingSelectFlags_ {
	...
	ImGuiTypingSelectFlags_AllowBackspace = 1
		<< 0, // Backspace to delete character inputs. If using: ensure
	...
	ImGuiTypingSelectFlags_AllowSingleCharMode = 1
		<< 1, // Allow "single char" search mode which is activated when
...
enum ImGuiOldColumnFlags_ {
	...
	ImGuiOldColumnFlags_NoBorder = 1 << 0, // Disable column Diviseurders
	ImGuiOldColumnFlags_NoResize = 1
		<< 1, // Disable resizing columns when clicking on the Diviseurders
	...
	ImGuiOldColumnFlags_GrowParentContentsSize = 1
		<< 4, // Restore pre-1.51 behavior of extending the parent window
...
struct IMGUI_API ImGuiMultiSelectTempData {
	ImGuiMultiSelectIO
		IO; // MUST BE FIRST FIELD. Requests are set and returned
...
struct IMGUI_API ImGuiMultiSelectState {
	...
	ImGuiSelectionUserData
		NavIdItem; // SetNextItemSelectionUserData() value for
...
struct ImGuiViewportP : public ImGuiViewport {
	int BgFgDrawListsLastFrame
		[2]; // Last frame number the background (0) and
	...
	ImVec2 CalcWorkRectSize(
		const ImVec2 &inset_min, const ImVec2 &inset_max) const
	{
	...
	ImRect GetBuildWorkRect() const
	{
		...
		ImVec2 size =
			CalcWorkRectSize(BuildWorkInsetMin, BuildWorkInsetMax);
		return ImRect(pos.x, pos.y, pos.x + size.x, pos.y + size.y);
	}
};
...
struct ImGuiSettingsHandler {
	const char
		*TypeName; // Short description stored in .ini file. Disallowed
			   // characters: '[' ']'
...
enum ImGuiDebugLogFlags_ {
	...
	ImGuiDebugLogFlags_EventError = 1
		<< 0, // Error submitted by IM_ASSERT_USER_ERROR()
	...
	ImGuiDebugLogFlags_EventMask_ = ImGuiDebugLogFlags_EventError |
	...
	ImGuiDebugLogFlags_OutputToTestEngine = 1
		<< 21, // Also send output to Test Engine
};
...
struct ImGuiStackLevelInfo {
	...
	ImGuiDataType DataType : 8;
	char Desc
		[57]; // Arbitrarily sized buffer to hold a result (FIXME: could
...
struct ImGuiIDStackTool {
	int LastActiveFrame;
	int StackLevel; // -1: query stack and resize Results, >= 0: inDiviseurdual
			// stack level
...
struct ImGuiContext {
	...
	ImFontBaked
		*FontBaked; // Currently bound font at currently bound size. (==
	...
	char ContextName
		[16]; // Storage for a context name (to facilitate debugging
	...
	ImGuiWindow
		*HoveredWindow; // Window the mouse is hovering. Will typically
				// catch mouse inputs.
	ImGuiWindow *
	...
	ImGuiItemFlags
		CurrentItemFlags; // Value for currently appending items ==
	...
	// Shared stacks
	ImGuiCol
		DebugFlashStyleColorIdx; // (Keep close to ColorStack to share cache
					 // line)
	...
	ImGuiWindow
		*NavWindow; // Focused window for navigation. Could be called
			    // 'FocusedWindow'
	...
	ImGuiNavLayer
		NavLayer; // Focused layer (main scrolling layer, or menu/title
			  // bar layer)
	...
	ImGuiDir
		NavMoveClipDir; // FIXME-NAV: Describe the purpose of this better.
	...
	ImGuiTextBuffer
		LogBuffer; // Accumulation buffer when log to clipboard. This
	...
	float FramerateSecPerFrame
		[60]; // Calculate estimate of framerate for user
...
struct IMGUI_API ImGuiWindow {
	...
	ImGuiChildFlags
		ChildFlags; // Set when window is a child window. See enum
			    // ImGuiChildFlags_
	ImGuiViewportP *
	...
	bool HasCloseButton; // Set when the window has a close button (p_open !=
			     // NULL)
	signed char
		ResizeBorderHovered; // Current border being hovered for resize
		// (-1: none, otherwise 0-3)
	signed char
		ResizeBorderHeld; // Current border being held for resize (-1:
				  // none, otherwise 0-3)
	...
	ImGuiCond
		SetWindowPosAllowFlags : 8; // store acceptable condition flags for
		// SetNextWindowPos() use.
	ImGuiCond
		SetWindowSizeAllowFlags : 8; // store acceptable condition flags
	...
	ImVector<ImGuiID>
		IDStack; // ID stack. ID are hashes seeded with the value
			 // at the top of the stack. (In theory this should
			 // be in the TempData structure)
	ImGuiWindowTempData
		DC; // Temporary per-window data, reset at the beginning
	...
	ImRect ContentRegionRect; // FIXME: This is currently confusing/misleading. It
	...
	ImVec2ih
		HitTestHoleSize; // Define an optional rectangular hole where mouse
	...
	ImGuiID NavLastIds
		[ImGuiNavLayer_COUNT]; // Last known NavId for this window,
				       // per layer (0/1)
	ImRect NavRectRel
		[ImGuiNavLayer_COUNT]; // Reference rectangle, in window
				       // relative space
	...
	ImRect MenuBarRect() const
	{
		...
		return ImRect(
			Pos.x, y1, Pos.x + SizeFull.x, y1 + MenuBarHeight);
	}
...
enum ImGuiTabItemFlagsPrivate_ {
	...
	ImGuiTabItemFlags_Button = 1
		<< 21, // Used by TabItemButton, change the tab
	// item behavior to mimic a button
	ImGuiTabItemFlags_Invisible = 1
		<< 22, // To reserve space e.g. with ImGuiTabItemFlags_Leading
...
struct IMGUI_API ImGuiTabBar {
	...
	ImVec2 BackupCursorPos;
	ImGuiTextBuffer
		TabsNames; // For non-docking tab bar we re-append names in a
			   // contiguous buffer.
...
struct ImGuiTableColumn {
	...
	ImGuiTableColumnIdx
		IndexWithinEnabledSet; // Index within enabled/visible
...
struct ImGuiTableInstanceData {
	...
	{
		TableInstanceID = 0;
		LastOuterHeight = LastTopHeadersRowHeight = LastFrozenHeight =
			0.0f;
...
struct IMGUI_API ImGuiTable {
	...
	void *RawData; // Single allocation to hold Columns[], DisplayOrderToIndex[],
		// and RowCellData[]
	ImGuiTableTempData
		*TempData; // Transient data while table is active. Point
	...
	ImBitArrayPtr
		VisibleMaskByIndex; // Column Index -> IsVisibleX|IsVisibleY
	...
	ImGuiWindow *
		InnerWindow; // Window holding the table data (== OuterWindow or
			     // a child window)
	...
	ImGuiTableSortSpecs
		SortSpecs; // Public facing sorts specs, this is what we
	...
	ImGuiTableColumnIdx
		ResizedColumn; // Index of column being resized. Reset
	...
	ImGuiTableColumnIdx
		FreezeColumnsCount; // Actual frozen columns count (==
				    ...
				    // no scrolling offset)
	ImGuiTableColumnIdx
		RowCellDataCurrent; // Index of current RowCellData[]
...
IMGUI_API bool IsWindowChildOf(ImGuiWindow *window,
	ImGuiWindow *potential_parent, bool popup_hierarchy);
...
inline void SetWindowParentWindowForFocusRoute(
...
IMGUI_API bool ItemHoverable(
	const ImRect &bb, ImGuiID id, ImGuiItemFlags item_flags);
IMGUI_API bool IsWindowContentHoverable(
...
IMGUI_API bool BeginPopupMenuEx(
...
IMGUI_API bool IsMouseClicked(
	ImGuiMouseButton button, ImGuiInputFlags flags, ImGuiID owner_id = 0);
...
IMGUI_API void RenderDragDropTargetRect(
...
// "widgets_typingselect" in imgui_test_suite for usage code. Please let us
...
IMGUI_API void TableAngledHeadersRowEx(ImGuiID row_id, float angle,
	float max_label_width, const ImGuiTableHeaderData *data,
	int data_count);
...
IMGUI_API ImGuiID TableGetColumnResizeID(
	ImGuiTable *table, int column_n, int instance_no = 0);
...
IMGUI_API void TabItemSpacing(
	const char *str_id, ImGuiTabItemFlags flags, float width);
IMGUI_API ImVec2 TabItemCalcSize(
...
IMGUI_API void TextAligned(float align_x, float size_x, const char *fmt,
	...); // FIXME-WIP: Works but API is likely to be reworked. This
	      // is designed for 1 item on the line. (#7024)
IMGUI_API void TextAlignedV(
...
IMGUI_API bool DataTypeApplyFromText(const char *buf, ImGuiDataType data_type,
	void *p_data, const char *format, void *p_data_when_empty = NULL);
...
// Debug Tools
IMGUI_API void DebugAllocHook(ImGuiDebugAllocInfo *info, int frame_count,
	void *ptr,
	size_t size); // size >= 0 : alloc, size = -1 : free
IMGUI_API void DebugDrawCursorPos(ImU32 col = IM_COL32(255, 0, 0, 255));
IMGUI_API void DebugDrawLineExtents(ImU32 col = IM_COL32(255, 0, 0, 255));
IMGUI_API void DebugDrawItemRect(ImU32 col = IM_COL32(255, 0, 0, 255));
IMGUI_API void DebugTextUnformattedWithLocateItem(
...
IMGUI_API bool DebugBreakButton(
	const char *label, const char *description_of_location);
IMGUI_API void DebugBreakButtonTooltip(
...
IMGUI_API void DebugNodeFontGlyphesForSrcMask(
...
// We avoid dragging imstb_rectpack.h into public header (partly because
// binding generators are having issues with it)
...
IMGUI_API void ImFontAtlasBuildSetupFontLoader(
...
IMGUI_API void ImFontAtlasBuildRenderBitmapFromString(ImFontAtlas *atlas, int x,
	int y, int w, int h, const char *in_str, char in_marker_char);
IMGUI_API void ImFontAtlasBuildClear(
	ImFontAtlas *atlas); // Clear output and custom rects

IMGUI_API ImTextureData *ImFontAtlasTextureAdd(
...
IMGUI_API void ImFontAtlasTextureGrow(
	ImFontAtlas *atlas, int old_w = -1, int old_h = -1);
...
IMGUI_API void ImFontAtlasBuildGetOversampleFactors(ImFontConfig *src,
	ImFontBaked *baked, int *out_oversample_h, int *out_oversample_v);
IMGUI_API void ImFontAtlasBuildDiscardBakes(
	ImFontAtlas *atlas, int unused_frames);

IMGUI_API bool ImFontAtlasFontSourceInit(ImFontAtlas *atlas, ImFontConfig *src);
IMGUI_API void ImFontAtlasFontSourceAddToFont(
	ImFontAtlas *atlas, ImFont *font, ImFontConfig *src);
IMGUI_API void ImFontAtlasFontDestroySourceData(
...
IMGUI_API void ImFontAtlasFontDiscardBakes(
	ImFontAtlas *atlas, ImFont *font, int unused_frames);

IMGUI_API ImGuiID ImFontAtlasBakedGetId(
...
IMGUI_API ImFontBaked *ImFontAtlasBakedGetOrAdd(ImFontAtlas *atlas,
	ImFont *font, float font_size, float font_rasterizer_density);
...
IMGUI_API void ImFontAtlasBakedDiscard(
...
IMGUI_API void ImFontAtlasBakedDiscardFontGlyph(ImFontAtlas *atlas,
	ImFont *font, ImFontBaked *baked, ImFontGlyph *glyph);
IMGUI_API void ImFontAtlasBakedSetFontGlyphBitmap(ImFontAtlas *atlas,
	ImFontBaked *baked, ImFontConfig *src, ImFontGlyph *glyph,
...
IMGUI_API ImFontAtlasRectId ImFontAtlasPackAddRect(ImFontAtlas *atlas, int w,
	int h, ImFontAtlasRectEntry *overwrite_entry = NULL);
IMGUI_API ImTextureRect *ImFontAtlasPackGetRect(
	ImFontAtlas *atlas, ImFontAtlasRectId id);
IMGUI_API ImTextureRect *ImFontAtlasPackGetRectSafe(
	ImFontAtlas *atlas, ImFontAtlasRectId id);
IMGUI_API void ImFontAtlasPackDiscardRect(
	ImFontAtlas *atlas, ImFontAtlasRectId id);

IMGUI_API void ImFontAtlasUpdateNewFrame(
	ImFontAtlas *atlas, int frame_count, bool renderer_has_textures);
IMGUI_API void ImFontAtlasAddDrawListSharedData(
	ImFontAtlas *atlas, ImDrawListSharedData *data);
IMGUI_API void ImFontAtlasRemoveDrawListSharedData(
	ImFontAtlas *atlas, ImDrawListSharedData *data);
...
IMGUI_API void ImFontAtlasTextureBlockCopy(ImTextureData *src_tex, int src_x,
	int src_y, ImTextureData *dst_tex, int dst_x, int dst_y, int w, int h);
...
#ifdef IMGUI_ENABLE_TEST_ENGINE
extern void ImGuiTestEngineHook_ItemAdd(ImGuiContext *ctx, ImGuiID id,
	const ImRect &bb,
