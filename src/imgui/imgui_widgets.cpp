...
void ImGui::TextEx(const char *text, const char *text_end, ImGuiTextFlags flags)
{
	...
	if (text_end - text <= 2000 || wrap_enabled) {
		...
		if (line < text_end) {
			ImRect line_rect(
				pos, pos + ImVec2(FLT_MAX, line_height));
			while (line < text_end) {
...
void ImGui::LabelTextV(const char *label, const char *fmt, va_list args)
{
	...
	ImFormatStringToTempBufferV(
		&value_text_begin, &value_text_end, fmt, args);
...
//   down)       -             true             true            - - - Frame N+4
//   (mouse moves outside bb)     -             -                true - - -
//   Frame N+5 (mouse moves inside bb)      -             true             true
//   -                  -                    - Frame N+6 (mouse button is
//   released)   true          true             -               - true - Frame
	...
	bool *out_held, ImGuiButtonFlags flags)
{
	...
	// Mouse handling
	const ImGuiID test_owner_id = (flags & ImGuiButtonFlags_NoTestKeyOwner)
		? ImGuiKeyOwner_Any
		: id;
	if (hovered) {
	...
	if (g.NavActivateDownId == id) {
		...
		if (!nav_activated_by_inputs &&
			...
			const float t1 = ImMax(
				ImMax(key1->DownDuration, key2->DownDuration),
				key3->DownDuration);
			nav_activated_by_inputs =
				CalcTypematicRepeatAmount(t1 - g.IO.DeltaTime,
					t1, g.IO.KeyRepeatDelay,
...
// stack. Then you can keep 'str_id' empty or the same for all your buttons
...
bool ImGui::ScrollbarEx(const ImRect &bb_frame, ImGuiID id, ImGuiAxis axis,
	ImS64 *p_scroll_v, ImS64 size_visible_v, ImS64 size_contents_v,
	ImDrawFlags draw_rounding_flags)
{
	...
	if ((axis == ImGuiAxis_Y) && bb_frame_height < bb_frame_width)
		alpha = ImSaturate(
			bb_frame_height / ImMax(bb_frame_width * 2.0f, 1.0f));
	if (alpha <= 0.0f)
		return false;
	...
	const float grab_h_minsize =
		ImMin(bb.GetSize()[axis], style.GrabMinSize);
	...
	if (held && allow_interaction && grab_h_norm < 1.0f) {
		...
		const int held_dir = (clicked_v_norm < grab_v_norm)    ? -1
			: (clicked_v_norm > grab_v_norm + grab_h_norm) ? +1
								       : 0;
		if (g.ActiveIdIsJustActivated) {
			// On initial click when held_dir == 0 (clicked over grab): calculate
			// the distance between mouse and the center of the grab
			const bool scroll_to_clicked_location =
				(g.IO.ConfigScrollbarScrollByPage == false ||
					g.IO.KeyShift || held_dir == 0);
			g.ScrollbarSeekMode = scroll_to_clicked_location
				? 0
				: (short)held_dir;
		...
		if (g.ScrollbarSeekMode == 0) {
			...
			*p_scroll_v = (ImS64)(scroll_v_norm * scroll_max);
			...
			if (IsMouseClicked(ImGuiMouseButton_Left,
				...
				float page_dir = (g.ScrollbarSeekMode > 0.0f)
					? +1.0f
					: -1.0f;
				*p_scroll_v = ImClamp(*p_scroll_v +
						(ImS64)(page_dir *
							size_visible_v),
					(ImS64)0, scroll_max);
			}
		...
		grab_v_norm = scroll_ratio *
			(scrollbar_size_v - grab_h_pixels) / scrollbar_size_v;
	...
	if (axis == ImGuiAxis_X)
		...
		grab_rect = ImRect(bb.Min.x,
			ImLerp(bb.Min.y, bb.Max.y, grab_v_norm), bb.Max.x,
	...
	const ImVec4 &tint_col)
{
	...
	if (g.Style.ImageBorderSize > 0.0f)
		window->DrawList->AddRect(bb.Min, bb.Max,
			GetColorU32(ImGuiCol_Border), 0.0f, ImDrawFlags_None,
			g.Style.ImageBorderSize);
	if (bg_col.w > 0.0f)
		window->DrawList->AddRectFilled(bb.Min + padding,
			bb.Max - padding, GetColorU32(bg_col));
	...
	const ImVec4 &border_col)
{
	...
	ImageWithBg(
		tex_ref, image_size, uv0, uv1, ImVec4(0, 0, 0, 0), tint_col);
	PopStyleColor();
	PopStyleVar();
}
#endif
...
bool ImGui::Checkbox(const char *label, bool *v)
{
				...
				total_bb)) // Extra layer of "no logic clip" for box-select support
		{
			IMGUI_TEST_ENGINE_ITEM_INFO(id, label,
				g.LastItemData.StatusFlags |
					...
					(*v ? ImGuiItemStatusFlags_Checked
					    : 0));
			return false;
		}
	...
	if (is_visible) {
		...
		if (mixed_value) {
			...
			const float pad =
				ImMax(1.0f, IM_TRUNC(square_sz / 6.0f));
	...
	float fraction, const ImVec2 &size_arg, const char *overlay)
{
	...
	if (is_indeterminate) {
		...
		fill_n0 = ImFmod(-fraction, 1.0f) * (1.0f + fill_width_n) -
			fill_width_n;
	...
	if (!is_indeterminate || overlay != NULL) {
		if (!overlay) {
			ImFormatString(overlay_buf, IM_ARRAYSIZE(overlay_buf),
...
bool ImGui::TextLink(const char *label)
{
	...
	ImVec4 line_colf = text_colf;
	{
		...
		// style as some earlier widgets, as we are currently
		// experimenting/planning a different styling system.
		float h, s, v;
		ColorConvertRGBtoHSV(
			text_colf.x, text_colf.y, text_colf.z, h, s, v);
		if (held || hovered) {
			v = ImSaturate(v + (held ? 0.4f : 0.3f));
			h = ImFmod(h + 0.02f, 1.0f);
		}
		ColorConvertHSVtoRGB(
			h, s, v, text_colf.x, text_colf.y, text_colf.z);
		v = ImSaturate(v - 0.20f);
		ColorConvertHSVtoRGB(
			h, s, v, line_colf.x, line_colf.y, line_colf.z);
	}

	float line_y = bb.Max.y +
		ImFloor(g.FontBaked->Descent * g.FontBakedScale * 0.20f);
	window->DrawList->AddLine(ImVec2(bb.Min.x, line_y),
		ImVec2(bb.Max.x, line_y),
		GetColorU32(
			line_colf)); // FIXME-TEXT: Underline mode // FIXME-DPI
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness)
{
	...
	if (flags & ImGuiSeparatorFlags_Vertical) {
		...
		// same feature for tables because tables naturally have border features.
...
void ImGui::SeparatorTextEx(
	ImGuiID id, const char *label, const char *label_end, float extra_w)
{
	ImGuiContext &g = *GImGui;
	ImGuiWindow *window = g.CurrentWindow;
	...
	const ImRect bb(
		pos, ImVec2(window->WorkRect.Max.x, pos.y + min_size.y));
	...
	ImGuiShrinkWidthItem *items, int count, float width_excess)
{
	...
	while (width_excess > 0.0f)
		for (int n = 0; n < count && width_excess > 0.0f; n++) {
			float width_to_add = ImMin(
				items[n].InitialWidth - items[n].Width, 1.0f);
	...
	const char *label, const char *preview_value, ImGuiComboFlags flags)
{
	...
	g.NextWindowData
		.ClearFlags(); // We behave like Begin() and need to consume
			       // those values
	if (window->SkipItems)
		return false;
	...
	if (!(flags & ImGuiComboFlags_NoPreview))
		window->DrawList->AddRectFilled(bb.Min,
			ImVec2(value_x2, bb.Max.y), frame_col,
			style.FrameRounding,
	...
	if (!(flags & ImGuiComboFlags_NoArrowButton)) {
		...
		window->DrawList->AddRectFilled(ImVec2(value_x2, bb.Min.y),
			bb.Max, bg_col, style.FrameRounding,
	...
	ImGuiID popup_id, const ImRect &bb, ImGuiComboFlags flags)
{
	...
	if (g.NextWindowData.HasFlags &
		...
		ImVec2 constraint_min(0.0f, 0.0f),
			constraint_max(FLT_MAX, FLT_MAX);
		if ((g.NextWindowData.HasFlags &
			    ImGuiNextWindowDataFlags_HasSize) == 0 ||
	...
	if (ImGuiWindow *popup_window = FindWindowByName(name))
		if (popup_window->WasActive) {
			// Always override 'AutoPosLastDirection' to not leave a chance for a
			// past value to affect us.
			ImVec2 size_expected =
				CalcWindowNextAutoFitSize(popup_window);
...
// Call directly after the BeginCombo/EndCombo block. The preview is designed
// to only host non-interactive elements (Experimental, see GitHub issues:
// #1658, #4168)
bool ImGui::BeginComboPreview()
{
	...
	window->DC.CursorPos =
		preview_data->PreviewRect.Min + g.Style.FramePadding;
	window->DC.CursorMaxPos = window->DC.CursorPos;
	window->DC.LayoutType = ImGuiLayoutType_Horizontal;
	window->DC.IsSameLine = false;
	PushClipRect(preview_data->PreviewRect.Min,
		preview_data->PreviewRect.Max, true);
...
void ImGui::EndComboPreview()
{
	...
	if (window->DC.CursorMaxPos.x < preview_data->PreviewRect.Max.x &&
			...
			1) // Unlikely case that the PushClipRect() didn't create a command
		{
			draw_list->_CmdHeader.ClipRect =
				draw_list
					->CmdBuffer[draw_list->CmdBuffer.Size -
						1]
					.ClipRect =
					draw_list
						->CmdBuffer[draw_list->CmdBuffer
								    .Size -
							2]
						.ClipRect;
	...
	window->DC.CursorMaxPos = ImMax(
		window->DC.CursorMaxPos, preview_data->BackupCursorMaxPos);
	...
	const char *items_separated_by_zeros, int height_in_items)
{
	...
	bool value_changed = Combo(label, current_item,
		Items_SingleStringGetter, (void *)items_separated_by_zeros,
...
static const char *ImGuiGetNameFromIndexOldToNewCallback(
	...
	int items_count, int height_in_items)
{
	...
	return ListBox(label, current_item,
		ImGuiGetNameFromIndexOldToNewCallback, &old_to_new_data,
		items_count, height_in_items);
}
bool ImGui::Combo(const char *label, int *current_item,
	bool (*old_getter)(void *, int, const char **), void *user_data,
...
// NB: This is _not_ a full expression evaluator. We should probably add one
// and replace this dumb mess..
bool ImGui::DataTypeApplyFromText(const char *buf, ImGuiDataType data_type,
	void *p_data, const char *format, void *p_data_when_empty)
{
	...
	if (!buf[0]) {
		if (p_data_when_empty != NULL) {
			...
			return memcmp(&data_backup, p_data, type_info->Size) !=
				0;
		}
		return false;
	}
	...
	if (data_type == ImGuiDataType_Float ||
		...
		format = ImParseFormatSanitizeForScanning(format,
			format_sanitized, IM_ARRAYSIZE(format_sanitized));
	...
	const char *format, ImGuiDataType data_type, TYPE v)
{
	...
	if (fmt_start[0] != '%' ||
		fmt_start[1] ==
			'%') // Don't apply if the value is not visible in the
			     // format string
		return v;
	...
	ImGuiSliderFlags flags)
{
	...
	const bool is_bounded = (v_min < v_max) ||
		((v_min == v_max) &&
			(v_min != 0.0f ||
				(flags & ImGuiSliderFlags_ClampZeroRange)));
	...
	if (g.ActiveIdSource == ImGuiInputSource_Mouse && IsMousePosValid() &&
		...
		const float tweak_factor =
			(flags & ImGuiSliderFlags_NoSpeedTweaks) ? 1.0f
	...
	if (is_logarithmic) {
		...
		v_cur = ScaleValueFromRatioT<TYPE, SIGNEDTYPE, FLOATTYPE>(
			data_type, v_new_parametric, v_min, v_max,
	...
	ImGuiSliderFlags flags)
{
	...
	case ImGuiDataType_S8: {
		...
		bool r = DragBehaviorT<ImS32, ImS32, float>(ImGuiDataType_S32,
			&v32, v_speed, p_min ? *(const ImS8 *)p_min : IM_S8_MIN,
			p_max ? *(const ImS8 *)p_max : IM_S8_MAX, format,
			flags);
		if (r)
			*(ImS8 *)p_v = (ImS8)v32;
		return r;
	...
	case ImGuiDataType_U8: {
		...
		bool r = DragBehaviorT<ImU32, ImS32, float>(ImGuiDataType_U32,
			&v32, v_speed, p_min ? *(const ImU8 *)p_min : IM_U8_MIN,
			p_max ? *(const ImU8 *)p_max : IM_U8_MAX, format,
			flags);
		if (r)
			*(ImU8 *)p_v = (ImU8)v32;
		return r;
	...
	case ImGuiDataType_S16: {
		...
		bool r = DragBehaviorT<ImS32, ImS32, float>(ImGuiDataType_S32,
			...
			p_max ? *(const ImS16 *)p_max : IM_S16_MAX, format,
			flags);
		if (r)
			*(ImS16 *)p_v = (ImS16)v32;
		return r;
	...
	case ImGuiDataType_U16: {
		...
		bool r = DragBehaviorT<ImU32, ImS32, float>(ImGuiDataType_U32,
			...
			p_max ? *(const ImU16 *)p_max : IM_U16_MAX, format,
			flags);
		if (r)
			*(ImU16 *)p_v = (ImU16)v32;
		return r;
	...
	case ImGuiDataType_Double:
		return DragBehaviorT<double, double, double>(data_type,
			...
			p_max ? *(const double *)p_max : DBL_MAX, format,
			flags);
	case ImGuiDataType_COUNT:
		break;
	}
	...
	ImGuiSliderFlags flags)
{
	...
	if (temp_input_is_active) {
		...
		return TempInputScalar(frame_bb, id, label, data_type, p_data,
			format, clamp_enabled ? p_min : NULL,
	...
	IMGUI_TEST_ENGINE_ITEM_INFO(id, label,
		g.LastItemData.StatusFlags |
			(temp_input_allowed ? ImGuiItemStatusFlags_Inputable
					    : 0));
	return value_changed;
}
	...
	const char *format, const char *format_max, ImGuiSliderFlags flags)
{
	...
	bool value_changed = DragScalar("##min", ImGuiDataType_Float,
		v_current_min, v_speed, &min_min, &min_max, format, min_flags);
	PopItemWidth();
	SameLine(0, g.Style.ItemInnerSpacing.x);
	...
	const char *format, const char *format_max, ImGuiSliderFlags flags)
{
	...
	ImGuiSliderFlags max_flags =
		flags | ((max_min == max_max) ? ImGuiSliderFlags_ReadOnly : 0);
	value_changed |= DragInt("##max", v_current_max, v_speed, max_min,
	...
	float zero_deadzone_halfsize)
{
	...
	if (is_logarithmic) {
		...
		float t_with_flip = flipped
			? (1.0f - t)
			: t; // t, but flipped if necessary to account
	...
	const char *format, ImGuiSliderFlags flags, ImRect *out_grab_bb)
{
	...
	const bool is_floating_point = (data_type == ImGuiDataType_Float) ||
		(data_type == ImGuiDataType_Double);
	const float v_range_f = (float)(v_min < v_max
			? v_max - v_min
	...
	if (g.ActiveId == id) {
		...
		if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
			if (!g.IO.MouseDown[0]) {
				...
				if (g.ActiveIdIsJustActivated) {
					float grab_t = ScaleRatioFromValueT<
			...
			if (g.NavActivatePressedId == id &&
				...
				clicked_t = ScaleRatioFromValueT<TYPE,
					SIGNEDTYPE, FLOATTYPE>(data_type, *v,
				...
				// past the limits
				{
					...
					// the accumulator
					TYPE v_new = ScaleValueFromRatioT<TYPE,
						SIGNEDTYPE, FLOATTYPE>(
						data_type, clicked_t, v_min,
		...
		if (set_new_value) {
			TYPE v_new = ScaleValueFromRatioT<TYPE, SIGNEDTYPE,
				FLOATTYPE>(data_type, clicked_t, v_min, v_max,
				is_logarithmic, logarithmic_zero_epsilon,
				zero_deadzone_halfsize);
...
// fail, but an integer Slider between INT_MAX/2-10 and INT_MAX/2 will be ok.
// It would be possible to lift that limitation with some work but it doesn't
// seem to be worth it for sliders.
bool ImGui::SliderBehavior(const ImRect &bb, ImGuiID id,
	ImGuiDataType data_type, void *p_v, const void *p_min,
	const void *p_max, const char *format, ImGuiSliderFlags flags,
	ImRect *out_grab_bb)
{
	// Read imgui.cpp "API BREAKING CHANGES" section for 1.78 if you hit this
	...
	case ImGuiDataType_Float:
		...
		return SliderBehaviorT<float, float, float>(bb, id, data_type,
			(float *)p_v, *(const float *)p_min,
			*(const float *)p_max, format, flags, out_grab_bb);
	case ImGuiDataType_Double:
		IM_ASSERT(*(const double *)p_min >= -DBL_MAX / 2.0f &&
			*(const double *)p_max <= DBL_MAX / 2.0f);
		return SliderBehaviorT<double, double, double>(bb, id,
			data_type, (double *)p_v, *(const double *)p_min,
...
// SliderInt() etc. or examples in 'Demo->Widgets->Data Types' to understand
	...
	ImGuiSliderFlags flags)
{
	...
	const bool value_changed = SliderBehavior(frame_bb, id, data_type,
		p_data, p_min, p_max, format, flags, &grab_bb);
	if (value_changed)
		MarkItemEdited(id);
	...
	const void *p_max, const char *format, ImGuiSliderFlags flags)
{
	...
	const ImRect frame_bb(
		window->DC.CursorPos, window->DC.CursorPos + size);
	...
	const char *fmt, char *buf, size_t buf_size)
{
	...
	if (fmt_end[0] ==
		0) // If we only have leading decoration, we don't need to
		   // copy the data.
		return fmt_start;
...
// ! "%07I64d" -> "%I64d"
const char *ImParseFormatSanitizeForScanning(
	const char *fmt_in, char *fmt_out, size_t fmt_out_size)
{
	const char *fmt_end = ImParseFormatFindEnd(fmt_in);
	...
	char *buf, int buf_size, ImGuiInputTextFlags flags)
{
	...
	bool value_changed = InputTextEx(label, NULL, buf, buf_size,
		bb.GetSize(), flags | ImGuiInputTextFlags_MergedItem);
	if (init) {
	...
	const void *p_clamp_min, const void *p_clamp_max)
{
	...
	if (TempInputText(
		...
		DataTypeApplyFromText(
			data_buf, data_type, p_data, format, NULL);
		if (p_clamp_min || p_clamp_max) {
			if (p_clamp_min && p_clamp_max &&
				DataTypeCompare(data_type, p_clamp_min,
					p_clamp_max) > 0)
...
// the data. For an Input widget, p_step and p_step_fast are optional. Read
// code of e.g. InputFloat(), InputInt() etc. or examples in
	...
	const char *format, ImGuiInputTextFlags flags)
{
	...
	if ((flags & ImGuiInputTextFlags_DisplayEmptyRefVal) &&
		...
		DataTypeFormatString(
			buf, IM_ARRAYSIZE(buf), data_type, p_data, format);
	...
	if (p_step == NULL) {
		if (InputText(label, buf, IM_ARRAYSIZE(buf), flags))
			value_changed = DataTypeApplyFromText(buf, data_type,
				p_data, format,
...
static ImVec2 InputTextCalcTextSize(ImGuiContext *ctx, const char *text_begin,
	const char *text_end, const char **remaining, ImVec2 *out_offset,
...
static int IMSTB_TEXTEDIT_GETNEXTCHARINDEX_IMPL(
	ImGuiInputTextState *obj, int idx)
{
	if (idx >= obj->TextLen)
...
static int IMSTB_TEXTEDIT_GETPREVCHARINDEX_IMPL(
	ImGuiInputTextState *obj, int idx)
{
	if (idx <= 0)
		return -1;
	const char *p = ImTextFindPreviousUtf8Codepoint(
		obj->TextSrc, obj->TextSrc + idx);
	...
	const char *new_text, int new_text_len)
{
	...
	if (new_text_len + text_len + 1 > obj->TextA.Size) {
		...
		obj->TextA.resize(text_len +
			ImClamp(new_text_len, 32, ImMax(256, new_text_len)) +
			1);
...
// separate records, so we perform it manually. (FIXME: Report to
// nothings/stb?)
static void stb_textedit_replace(ImGuiInputTextState *str,
	STB_TexteditState *state, const IMSTB_TEXTEDIT_CHARTYPE *text,
	int text_len)
{
	...
	void *user_data, bool input_source_is_clipboard)
{
	...
	if (c < 0x20) {
		...
		if (!pass)
			return false;
		apply_named_filters =
			false; // Override named filters below so newline
	...
	if (apply_named_filters &&
		...
		// Users of non-default decimal point (in particular ',') may be affected
		// by word-selection logic
...
static void InputTextReconcileUndoState(ImGuiInputTextState *state,
	const char *old_buf, int old_length, const char *new_buf,
	int new_length)
{
	...
	if (insert_len > 0 || delete_len > 0)
		if (IMSTB_TEXTEDIT_CHARTYPE *p =
				stb_text_createundo(&state->Stb->undostate,
					first_diff, delete_len, insert_len))
...
void ImGui::InputTextDeactivateHook(ImGuiID id)
{
	...
	if (state->Flags & ImGuiInputTextFlags_ReadOnly) {
		...
		memcpy(g.InputTextDeactivatedState.TextA.Data,
			state->TextA.Data, state->TextLen + 1);
	}
}
	...
	ImGuiInputTextCallback callback, void *callback_user_data)
{
	...
	if (is_multiline) {
		...
		bool child_visible = BeginChildEx(label, id, frame_bb.GetSize(),
	...
	if (g.ActiveId == id) {
		...
		// inDiviseurdual shortcuts.
		const ImGuiKey always_owned_keys[] = {ImGuiKey_LeftArrow,
			ImGuiKey_RightArrow, ImGuiKey_Enter,
			ImGuiKey_KeypadEnter, ImGuiKey_Delete,
	...
	bool render_selection = state &&
		(state->HasSelection() || select_all) &&
	...
	const bool buf_display_from_state =
		...
		!is_readonly && state;
	bool is_displaying_hint = (hint != NULL &&
	...
	if (g.ActiveId == id) {
		...
		// line we should have a cleaner library-wide concept of Selected vs
		// Active.
		...
		if (select_all) {
			...
			if ((multiclick_count % 2) == 0) {
				...
				if (!STB_TEXT_HAS_SELECTION(state->Stb))
					ImStb::stb_textedit_prep_selection_at_cursor(
						state->Stb);
				state->Stb->cursor =
					ImStb::STB_TEXTEDIT_MOVEWORDRIGHT_MAC(
			...
			if (hovered) {
				if (io.KeyShift)
					stb_textedit_drag(state, state->Stb,
						mouse_x, mouse_y);
				else
					stb_textedit_click(state, state->Stb,
						mouse_x, mouse_y);
				state->CursorAnimReset();
			}
		...
		if ((flags & ImGuiInputTextFlags_AllowTabInput) &&
			!is_readonly) {
			if (Shortcut(
				    ImGuiKey_Tab, ImGuiInputFlags_Repeat, id)) {
				unsigned int c = '\t'; // Insert TAB
				if (InputTextFilterCharacter(&g, &c, flags,
			...
			if (!ignore_char_inputs && !is_readonly &&
				...
				for (int n = 0;
					...
					if (c == '\t') // Skip Tab, see above.
						continue;
					if (InputTextFilterCharacter(&g, &c,
						    flags, callback,
						    callback_user_data))
						state->OnCharPressed(c);
				}
	...
	if (g.ActiveId == id && !g.ActiveIdIsJustActivated &&
		...
		const int row_count_per_page =
			ImMax((int)((inner_size.y - style.FramePadding.y) /
				      g.FontSize),
				1);
		...
		// use CTRL+A and CTRL+B: former would be handled by InputText) Otherwise
		// we could simply assume that we own the keys as we are active.
		const ImGuiInputFlags f_repeat = ImGuiInputFlags_Repeat;
		const bool is_cut =
			(Shortcut(ImGuiMod_Ctrl | ImGuiKey_X, f_repeat, id) ||
				Shortcut(ImGuiMod_Shift | ImGuiKey_Delete,
					f_repeat, id)) &&
			!is_readonly && !is_password &&
			(!is_multiline || state->HasSelection());
		const bool is_copy =
			(Shortcut(ImGuiMod_Ctrl | ImGuiKey_C, 0, id) ||
				Shortcut(ImGuiMod_Ctrl | ImGuiKey_Insert, 0,
					id)) &&
			!is_password &&
			(!is_multiline || state->HasSelection());
		const bool is_paste =
			(Shortcut(ImGuiMod_Ctrl | ImGuiKey_V, f_repeat, id) ||
				Shortcut(ImGuiMod_Shift | ImGuiKey_Insert,
					f_repeat, id)) &&
			!is_readonly;
		const bool is_undo =
		...
		const bool is_redo =
			(Shortcut(ImGuiMod_Ctrl | ImGuiKey_Y, f_repeat, id) ||
				Shortcut(ImGuiMod_Ctrl | ImGuiMod_Shift |
						ImGuiKey_Z,
					f_repeat, id)) &&
			!is_readonly && is_undoable;
		...
		const bool is_cancel =
			...
			(nav_gamepad_active &&
				Shortcut(ImGuiKey_NavGamepadCancel, f_repeat,
					id));
		...
		if (IsKeyPressed(ImGuiKey_LeftArrow)) {
			...
			if (!state->HasSelection()) {
				if (is_wordmove_key_down)
					state->OnKeyPressed(
						STB_TEXTEDIT_K_WORDLEFT |
						STB_TEXTEDIT_K_SHIFT);
				else if (is_osx && io.KeyCtrl && !io.KeyAlt &&
					!io.KeySuper)
					state->OnKeyPressed(
			...
			if (g.PlatformIO.Platform_SetClipboardTextFn != NULL) {
				...
				memcpy(g.TempBuffer.Data, state->TextSrc + ib,
					ie - ib);
			...
			if (const char *clipboard = GetClipboardText()) {
				...
				for (const char *s = clipboard; *s != 0;) {
					...
					clipboard_filtered.resize(
						clipboard_filtered.Size +
						out_len);
					memcpy(clipboard_filtered.Data +
							clipboard_filtered
								.Size -
							out_len,
						c_utf8, out_len);
	...
	if (g.ActiveId == id) {
		...
		if (revert_edit && !is_readonly) {
			if (flags & ImGuiInputTextFlags_EscapeClearsAll) {
				...
				stb_textedit_replace(
					state, state->Stb, &empty_string, 0);
			} else if (strcmp(buf, state->TextToRevertTo.Data) !=
				0) {
		...
		// 'validated == true' also writes back. This also allows the user to use
		// InputText() without maintaining any user-side storage. (please note
		// that if you use this property along ImGuiInputTextFlags_CallbackResize
		// you can end up with your temporary string object unnecessarily
		// allocating once a frame, either store your string data, either if you
		...
		if (apply_edit_back_to_user_buffer) {
			...
			// gets priority over any underlying modification of the input buffer
			...
			if ((flags &
				...
				if (event_flag) {
					...
					char *callback_buf = is_readonly
						? buf
						: state->TextA.Data;
					IM_ASSERT(
						callback_buf == state->TextSrc);
					state->CallbackTextBackup.resize(
						state->TextLen + 1);
					...
					if (buf_dirty) {
						// Callback may update buffer and thus set buf_dirty even
						// in read-only mode.
						IM_ASSERT(callback_data
								  .BufTextLen ==
							(int)ImStrlen(
								callback_data
	...
	if (g.InputTextDeactivatedState.ID == id) {
		if (g.ActiveId != id && IsItemDeactivatedAfterEdit() &&
			!is_readonly &&
			strcmp(g.InputTextDeactivatedState.TextA.Data, buf) !=
				0) {
	...
	if (apply_new_text != NULL) {
		...
		//// of our owned buffer matches the size of the string object held by
		/// the user, and by design we allow InputText() to be used / without any
	...
	const bool new_is_displaying_hint = (hint != NULL &&
		(buf_display_from_state ? state->TextA.Data : buf)[0] == 0);
	...
	if (render_cursor || render_selection) {
...

		{
			...
			const char *cursor_ptr = render_cursor
				...
				: NULL;
			const char *selmin_ptr = render_selection ? text_begin +
					ImMin(state->Stb->select_start,
						state->Stb->select_end)
								  : NULL;
			...
			cursor_offset.x = InputTextCalcTextSize(&g,
				ImStrbol(cursor_ptr, text_begin), cursor_ptr)
						  .x;
			cursor_offset.y = cursor_line_no * g.FontSize;
			if (selmin_line_no >= 0) {
				select_start_offset.x = InputTextCalcTextSize(
					&g, ImStrbol(selmin_ptr, text_begin),
					selmin_ptr)
								.x;
				select_start_offset.y =
					selmin_line_no * g.FontSize;
			}
			...
			if (is_multiline)
				text_size = ImVec2(
					inner_size.x, line_count * g.FontSize);
		}
		...
		if (render_cursor && state->CursorFollow) {
			...
			if (!(flags & ImGuiInputTextFlags_NoHorizontalScroll)) {
				...
				const float visible_width =
					inner_size.x - style.FramePadding.x;
				if (cursor_offset.x < state->Scroll.x)
					state->Scroll.x = IM_TRUNC(ImMax(0.0f,
						cursor_offset.x -
							scroll_increment_x));
				else if (cursor_offset.x - visible_width >=
					state->Scroll.x)
					state->Scroll.x =
						IM_TRUNC(cursor_offset.x -
							visible_width +
							scroll_increment_x);
			} else {
				state->Scroll.y = 0.0f;
		...
		if (render_selection) {
			...
			for (const char *p = text_selected_begin;
				...
				if (rect_pos.y < clip_rect.y) {
					...
					ImVec2 rect_size =
						InputTextCalcTextSize(&g, p,
							text_selected_end, &p,
							NULL, true);
					if (rect_size.x <= 0.0f)
						rect_size.x = IM_TRUNC(
							g.FontBaked->GetCharAdvance(
								(ImWchar)' ') *
		...
		if (is_multiline ||
			...
			draw_window->DrawList->AddText(g.Font, g.FontSize,
				draw_pos - draw_scroll, col, buf_display,
		...
		if (is_multiline ||
			...
			if (flags & ImGuiInputTextFlags_ElideLeft)
				draw_pos.x = ImMin(draw_pos.x,
					frame_bb.Max.x -
						CalcTextSize(buf_display, NULL)
							.x -
						style.FramePadding.x);

			const ImVec2 draw_scroll =
				/*state ? ImVec2(state->Scroll.x, 0.0f) :*/
				ImVec2(0.0f,
	...
	if (is_multiline) {
		...
		// gets a bit messy as EndGroup() tries to forward scrollbar being
		// active...
...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState *state)
{
	...
	Text("(Internal Buffer: TextA Size: %d, Capacity: %d)",
		state->TextA.Size, state->TextA.Capacity);
			    ...
			    ImGuiChildFlags_ResizeY)) // Visualize undo state
	{
		...
		for (int n = 0; n < IMSTB_TEXTEDIT_UNDOSTATECOUNT; n++) {
			ImStb::StbUndoRecord *undo_rec =
				&undo_state->undo_rec[n];
			...
			Text("%c [%02d] where %03d, insert %03d, delete %03d, "
			     "char_storage "
			     "%03d \"%.*s\"",
				undo_rec_type, n, undo_rec->where,
				undo_rec->insert_length,
	...
	if ((flags &
		...
		for (int n = 0; n < components; n++) {
			if (n > 0)
				SameLine(0, style.ItemInnerSpacing.x);
			float next_split =
				IM_TRUNC(w_items * (n + 1) / components);
			...
			if (flags & ImGuiColorEditFlags_Float) {
				value_changed |= DragFloat(ids[n], &f[n],
					1.0f / 255.0f, 0.0f, hdr ? 0.0f : 1.0f,
					fmt_table_float[fmt_idx][n]);
				value_changed_as_float |= value_changed;
			} else {
				value_changed |= DragInt(ids[n], &i[n], 1.0f, 0,
					hdr ? 0 : 255,
		...
		if (alpha)
			ImFormatString(buf, IM_ARRAYSIZE(buf),
				"#%02X%02X%02X%02X", ImClamp(i[0], 0, 255),
		...
		if (InputText("##Text", buf, IM_ARRAYSIZE(buf),
			...
			if (alpha)
				...
				r = sscanf(p, "%02X%02X%02X",
					(unsigned int *)&i[0],
					(unsigned int *)&i[1],
					(unsigned int *)&i[2]);
			IM_UNUSED(
				r); // Fixes C6031: Return value ignored: 'sscanf'.
		}
		if (!(flags & ImGuiColorEditFlags_NoOptions))
		...
		if (BeginPopup("picker")) {
			if (g.CurrentWindow->BeginCount == 1) {
				...
				value_changed |= ColorPicker4("##picker", col,
					picker_flags, &g.ColorPickerRef.x);
			}
			EndPopup();
		}
	}
	...
	ImVec2 half_sz, float bar_w, float alpha)
{
	...
	ImGui::RenderArrowPointingAt(draw_list,
		ImVec2(pos.x + half_sz.x, pos.y), half_sz, ImGuiDir_Right,
	...
	ImGui::RenderArrowPointingAt(draw_list,
		...
		IM_COL32(0, 0, 0, alpha8));
	ImGui::RenderArrowPointingAt(draw_list,
		ImVec2(pos.x + bar_w - half_sz.x, pos.y), half_sz,
	...
	ImGuiColorEditFlags flags, const float *ref_col)
{
	...
	ImVec2 triangle_pb = ImVec2(
		triangle_r * -0.5f, triangle_r * -0.866025f); // Black point.
	ImVec2 triangle_pc = ImVec2(
		triangle_r * -0.5f, triangle_r * +0.866025f); // White point.
	...
	if (flags & ImGuiColorEditFlags_PickerHueWheel) {
		...
		InvisibleButton("hsv",
			ImVec2(sv_picker_size + style.ItemInnerSpacing.x +
					bars_width,
				sv_picker_size));
		if (IsItemActive() && !is_readonly) {
			...
			if (initial_dist2 >= (wheel_r_inner - 1) *
						(wheel_r_inner - 1) &&
				initial_dist2 <= (wheel_r_outer + 1) *
						(wheel_r_outer + 1)) {
	...
	if ((flags & ImGuiColorEditFlags_NoInputs) == 0) {
		...
		if (flags & ImGuiColorEditFlags_DisplayRGB ||
			...
			if (ColorEdit4("##rgb", col,
				...
				// && !ActiveIdAllowOverlap) vs. using the InputText or DropTarget.
				// For the later we don't want to run the hue-wrap canceling code.
				// If you are well versed in HSV picker please provide your input!
				// (See #2050)
	...
	if (value_changed_fix_hue_wrap &&
		...
		if (new_H <= 0 && H > 0) {
			if (new_V <= 0 && V != new_V)
				ColorConvertHSVtoRGB(H, S,
					new_V <= 0 ? V * 0.5f : new_V, col[0],
					col[1], col[2]);
			else if (new_S <= 0)
				ColorConvertHSVtoRGB(H,
					new_S <= 0 ? S * 0.5f : new_S, new_V,
					col[0], col[1], col[2]);
		}
	}
	...
	ImU32 user_col32_striped_of_alpha = ColorConvertFloat4ToU32(ImVec4(R, G,
		B, style.Alpha)); // Important: this is still including the
	...
	if (flags & ImGuiColorEditFlags_PickerHueWheel) {
		...
		for (int n = 0; n < 6; n++) {
			...
			ShadeVertsLinearColorGradientKeepAlpha(draw_list,
				vert_start_idx, vert_end_idx, gradient_p0,
	...
	if (alpha_bar) {
		...
		RenderColorRectWithAlphaCheckerboard(draw_list, bar1_bb.Min,
			bar1_bb.Max, 0, bar1_bb.GetWidth() / 2.0f,
			ImVec2(0.0f, 0.0f));
		draw_list->AddRectFilledMultiColor(bar1_bb.Min, bar1_bb.Max,
			...
			user_col32_striped_of_alpha & ~IM_COL32_A_MASK);
		float bar1_line_y = IM_ROUND(
			picker_pos.y + (1.0f - alpha) * sv_picker_size);
	...
	if (g.ActiveId == id && !(flags & ImGuiColorEditFlags_NoDragDrop) &&
		...
		if (flags & ImGuiColorEditFlags_NoAlpha)
			SetDragDropPayload(IMGUI_PAYLOAD_TYPE_COLOR_3F,
				&col_rgb, sizeof(float) * 3, ImGuiCond_Once);
		else
			SetDragDropPayload(IMGUI_PAYLOAD_TYPE_COLOR_4F,
				&col_rgb, sizeof(float) * 4, ImGuiCond_Once);
...
void ImGui::ColorEditOptionsPopup(const float *col, ImGuiColorEditFlags flags)
{
	...
	if (allow_opt_datatype) {
		if (allow_opt_inputs)
			Separator();
		if (RadioButton(
			    "0..255", (opts & ImGuiColorEditFlags_Uint8) != 0))
	...
	if (BeginPopup("Copy")) {
		...
		char buf[64];
		ImFormatString(buf, IM_ARRAYSIZE(buf),
			"(%.3ff, %.3ff, %.3ff, %.3ff)", col[0], col[1], col[2],
			(flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : col[3]);
		if (Selectable(buf))
	...
	const char *label, const char *label_end)
{
	...
	const float text_offset_x = g.FontSize +
		(display_frame ? padding.x * 3
			       : padding.x *
					2); // Collapsing arrow width + Spacing
	...
	if (!is_visible) {
		if ((flags & ImGuiTreeNodeFlags_DrawLinesToNodes) &&
			...
			parent_data->DrawLinesToNodesY2 = ImMax(
				...
				window->DC.CursorPos
					.y); // Don't need to aim to mid Y position
					     // as we are clipped anyway.
			if (frame_bb.Min.y >= window->ClipRect.Max.y)
				window->DC.TreeRecordsClippedNodesY2Mask &= ~(1
					<< (window->DC.TreeDepth - 1)); // Done
		}
	...
	// Render
	{
		...
		if (display_frame) {
			...
			if (flags & ImGuiTreeNodeFlags_Bullet)
				...
				RenderArrow(window->DrawList,
					ImVec2(text_pos.x - text_offset_x +
							padding.x,
						text_pos.y),
					text_col,
					is_open ? ((flags &
			...
			if (hovered || selected) {
				const ImU32 bg_col =
					GetColorU32((held && hovered)
							...
							: ImGuiCol_Header);
				RenderFrame(frame_bb.Min, frame_bb.Max, bg_col,
					false);
			}
			RenderNavCursor(frame_bb, id, nav_render_cursor_flags);
			if (span_all_columns && !span_all_columns_label)
...
void ImGui::TreeNodeDrawLineToTreePop(const ImGuiTreeNodeStackData *data)
{
	...
	if (data->TreeFlags & ImGuiTreeNodeFlags_DrawLinesFull) {
		...
		y2_full = ImTrunc(
			y2_full - g.Style.ItemSpacing.y - g.FontSize * 0.5f);
...
// CollapsingHeader returns true when opened but do not indent nor push into
// the ID stack (because of the ImGuiTreeNodeFlags_NoTreePushOnOpen flag). This
// is basically the same as calling TreeNodeEx(label,
	...
	if (p_visible != NULL) {
		...
		float button_x = ImMax(g.LastItemData.Rect.Min.x,
			g.LastItemData.Rect.Max.x - g.Style.FramePadding.x -
				button_size);
		float button_y =
			g.LastItemData.Rect.Min.y + g.Style.FramePadding.y;
...
// ImGuiSelectableFlags_SpanAllColumns and ImGuiSelectableFlags_AllowOverlap
	...
	ImGuiSelectableFlags flags, const ImVec2 &size_arg)
{
	...
	// the fast path of override ClipRect + full push on render only, which would
	// be advantageous since most selectable are not selected.
	if (span_all_columns) {
		if (g.CurrentTable)
			TablePushBackgroundChannel();
...
// However the code is written in a way where calling it from multiple
// locations is safe (e.g. to obtain buffer).
	...
	ImGuiTypingSelectFlags flags)
{
	...
	for (ImWchar w : g.IO.InputQueueCharacters) {
		...
		if (data->SingleCharModeLock &&
			w_len == out_request->SingleCharSize &&
		...
		memcpy(data->SearchBuffer + buffer_len, w_buf,
			w_len + 1); // Append
		buffer_len += w_len;
		select_request = true;
	}
	...
	if (flags & ImGuiTypingSelectFlags_AllowSingleCharMode) {
		...
		const int c0_len =
			ImTextCountUtf8BytesFromChar(buf_begin, buf_end);
		...
		const int single_char_count = (p == buf_end)
			? (out_request->SearchBufferLen / c0_len)
			: 0;
...
// - it is better to NOT display a tooltip of other on-screen display
// indicator.
	...
	int nav_item_idx)
{
	...
	if (req->SingleCharMode &&
		...
		idx = TypingSelectFindNextSingleCharMatch(req, items_count,
...
int ImGui::TypingSelectFindNextSingleCharMatch(ImGuiTypingSelectRequest *req,
	int items_count, const char *(*get_item_name_func)(void *, int),
	void *user_data, int nav_item_idx)
{
	...
	for (int idx = 0; idx < items_count; idx++) {
		...
		if (first_match_idx == -1 &&
			nav_item_idx ==
				-1) // Return first match immediately if we don't
				    // have a nav_item_idx value.
			return idx;
		if (first_match_idx == -1) // Record first match for wrapping.
			first_match_idx = idx;
		if (nav_item_idx ==
			idx) // Record that we encountering nav_item so we
...
static void BoxSelectPreStartDrag(
	ImGuiID id, ImGuiSelectionUserData clicked_item)
{
	ImGuiContext &g = *GImGui;
	ImGuiBoxSelectState *bs = &g.BoxSelectState;
	bs->ID = id;
	bs->IsStarting = true; // Consider starting box-select.
	bs->IsStartedFromVoid =
		(clicked_item == ImGuiSelectionUserData_Invalid);
	...
	ImGuiBoxSelectState *bs, ImGuiWindow *window, const ImRect &inner_r)
{
	...
	for (int n = 0; n < 2; n++) // each axis
	{
		...
		const float speed_multiplier = ImLinearRemapClamp(g.FontSize,
			g.FontSize * 5.0f, 1.0f, 4.0f,
			ImAbs(dist)); // x1 to x4 depending on distance
		const float scroll_step = g.FontSize * 35.0f *
			speed_multiplier * ImSign(dist) * g.IO.DeltaTime;
	...
	ImGuiID box_select_id, ImGuiMultiSelectFlags ms_flags)
{
	...
	if (ms_flags &
		...
		curr_end_pos_abs = ImClamp(
			curr_end_pos_abs, scope_rect.Min, scope_rect.Max);
	...
	if (ms_flags & ImGuiMultiSelectFlags_BoxSelect2d)
		if (bs->BoxSelectRectPrev.Min.x !=
				bs->BoxSelectRectCurr.Min.x ||
			bs->BoxSelectRectPrev.Max.x !=
				bs->BoxSelectRectCurr.Max.x) {
			bs->UnclipMode = true;
			bs->UnclipRect =
...
static void DebugLogMultiSelectRequests(
	const char *function, const ImGuiMultiSelectIO *io)
{
	ImGuiContext &g = *GImGui;
	IM_UNUSED(function);
...
static ImRect CalcScopeRect(ImGuiMultiSelectTempData *ms, ImGuiWindow *window)
{
	...
	if (ms->Flags & ImGuiMultiSelectFlags_ScopeRect) {
		...
		// InnerClipRect?
		scope_rect.Min = ImMin(scope_rect.Min +
				ImVec2(window->DecoInnerSizeX1,
...
ImGuiMultiSelectIO *ImGui::EndMultiSelect()
{
	...
	if (ms->IsFocused) {
		...
		if (ms->NavIdPassedBy == false &&
			...
			IMGUI_DEBUG_LOG_SELECTION("[selection] EndMultiSelect: "
						  "Reset NavIdItem.\n");
			storage->NavIdItem = ImGuiSelectionUserData_Invalid;
	...
	ImGuiID id, bool *p_selected, ImGuiButtonFlags *p_button_flags)
{
	...
	if (ms->IsFocused) {
		...
		IM_ASSERT(
			g.NextItemData.FocusScopeId == g.CurrentFocusScopeId &&
			"Forgot to call SetNextItemSelectionUserData() prior "
			"to item, "
			"required in BeginMultiSelect()/EndMultiSelect() "
			"scope");

		// Apply SetAll (Clear/SelectAll) requests requested by
		// BeginMultiSelect(). This is only useful if the user hasn't processed
		// them already, and this only works if the user isn't using the clipper.
		// If you are using a clipper you need to process the SetAll request
		// after calling BeginMultiSelect()
		...
		// a frame of lag with the selection highlight (otherwise scrolling would
...
void ImGui::MultiSelectItemFooter(ImGuiID id, bool *p_selected, bool *p_pressed)
{
	...
	if (g.NavJustMovedToId == id) {
		if ((flags & ImGuiMultiSelectFlags_NoAutoSelect) == 0) {
			...
			if (is_shift)
				...
				apply_to_range_src =
					true; // Since if (pressed) {} main block is
					      // not running we update this
		}
	}
	...
	// Box-select toggle handling
	if (ms->BoxSelectId != 0)
		if (ImGuiBoxSelectState *bs =
				GetBoxSelectState(ms->BoxSelectId)) {
			const bool rect_overlap_curr =
				bs->BoxSelectRectCurr.Overlaps(
					g.LastItemData.Rect);
			const bool rect_overlap_prev =
				bs->BoxSelectRectPrev.Overlaps(
					g.LastItemData.Rect);
			if ((rect_overlap_curr && !rect_overlap_prev &&
				    !selected) ||
				...
				if (storage->LastSelectionSize <= 0 &&
					...
					selected = !selected;
					MultiSelectAddSetRange(ms, selected, +1,
						item_data, item_data);
				}
				storage->LastSelectionSize = ImMax(
					storage->LastSelectionSize + 1, 1);
			}
		}
	...
	// unless current item is not selected. The later, "unless current item is
	// not select", may become optional? It seems like a better default if Enter
	...
	if (pressed && (!enter_pressed || !selected)) {
		...
		// Navigated: Ctrl        | n/a    | n/a Keys Navigated:      Shift  |
		// n/a    | Dst=item, Sel=1,   => Clear + SetRange 1 Keys Navigated:
		// Ctrl+Shift  | n/a    | Dst=item, Sel=Src  => Clear + SetRange Src-Dst
		// Keys Activated:             | n/a    | Src=item, Sel=1    => Clear +
		// SetRange 1 Keys Activated: Ctrl        | n/a    | Src=item, Sel=!Sel
		// =>         SetSange 1 Keys Activated:      Shift  | n/a    | Dst=item,
		// Sel=1    => Clear + SetSange 1
		//----------------------------------------------------------------------------------------
		// Mouse Pressed:              | n/a    | Src=item, Sel=1,   => Clear +
		// SetRange 1 Mouse Pressed:  Ctrl        | n/a    | Src=item, Sel=!Sel
		// =>         SetRange 1 Mouse Pressed:       Shift  | n/a    | Dst=item,
		// Sel=1,   => Clear + SetRange 1 Mouse Pressed:  Ctrl+Shift  | n/a    |
...
{
	ImGuiSelectionRequest req = {ImGuiSelectionRequestType_SetAll, selected,
		0, ImGuiSelectionUserData_Invalid,
		ImGuiSelectionUserData_Invalid};
...
void ImGui::DebugNodeMultiSelectState(ImGuiMultiSelectState *storage)
{
	...
	if (!open)
		return;
	Text("RangeSrcItem = %" IM_PRId64 " (0x%" IM_PRIX64
	     "), RangeSelected = %d",
		storage->RangeSrcItem, storage->RangeSrcItem,
		storage->RangeSelected);
	Text("NavIdItem = %" IM_PRId64 " (0x%" IM_PRIX64
	     "), NavIdSelected = %d",
	...
	int size_before_amends, int selection_order)
{
	...
	ImGuiStoragePair *it = ImLowerBound(storage->Data.Data,
		storage->Data.Data + size_before_amends, id);
	const bool is_contained =
		(it != storage->Data.Data + size_before_amends) &&
		(it->key == id);
...
void ImGuiSelectionBasicStorage::ApplyRequests(ImGuiMultiSelectIO *ms_io)
{
	...
	// which is optional. It makes sense when using ImGuiSelectionBasicStorage to
	// simply pass your items count to BeginMultiSelect(). Other scheme may
	...
	const char *const items[], int items_count, int height_items)
{
	const bool value_changed = ListBox(label, current_item,
		Items_ArrayGetter, (void *)items, items_count, height_items);
	return value_changed;
}
	...
	int items_count, int height_in_items)
{
	...
	// different height, you can create a custom version of ListBox() in your
...
bool ImGui::BeginMenuBar()
{
	...
	const float border_top = ImMax(
		window->WindowBorderSize * 0.5f - window->TitleBarHeight, 0.0f);
...
static bool IsRootOfOpenMenuSet()
{
	...
	// loose menu items) based on parent ID. This would however prevent the use
	// of e.g. PushID() user code submitting menus. Previously this worked
	...
	// first child menu was focused - but this was generally a bug with other
	// side effects. Instead we don't treat Popup specifically (in order to
	...
	// IsRootOfOpenMenuSet() check to allow hovering between root window/popup
	// and first child menu. In the end, lack of ID check made it so we could no
	// longer differentiate between separate menu sets. To compensate for that,
	// we at least check parent window nav layer. This fixes the most common case
	// of menu opening on hover when moving between window content and menu bar.
...
bool ImGui::BeginMenuEx(const char *label, const char *icon, bool enabled)
{
	...
	// a suitable position for the child menu, However the final position is
	// going to be different! It is chosen by FindBestWindowPosForPopup(). e.g.
	// Menus tend to overlap each other horizontally to amplify relative
	// Z-ordering.
	...
	if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
		...
		float min_w =
			window->DC.MenuColumns.DeclColumns(icon_w, label_size.x,
				0.0f, checkmark_w); // Feedback to next frame
		...
		if (icon_w > 0.0f)
			RenderText(
				pos + ImVec2(offsets->OffsetIcon, 0.0f), icon);
		RenderArrow(window->DrawList,
			pos +
				ImVec2(offsets->OffsetMark + extra_w +
						g.FontSize * 0.30f,
					0.0f),
	...
	// (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
	{
		...
		if (g.HoveredWindow == window && child_menu_window != NULL) {
			...
			const float child_dir =
				(window->Pos.x < child_menu_window->Pos.x)
				? 1.0f
				: -1.0f;
			const ImRect next_window_rect =
				child_menu_window->Rect();
	...
	if (menu_is_open) {
		...
		if (menu_is_open) {
			...
			if (want_open && want_open_nav_init &&
				!g.NavInitRequest) {
	...
	const char *shortcut, bool selected, bool enabled)
{
	...
	// unsure whether this should be kept at all. For now moved it to be an
	...
	if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
		...
		// that in this situation: we don't render the shortcut, we render a
		// highlight instead of the selected tick mark.
		float w = label_size.x;
		window->DC.CursorPos.x += IM_TRUNC(style.ItemSpacing.x * 0.5f);
		ImVec2 text_pos(window->DC.CursorPos.x + offsets->OffsetLabel,
			window->DC.CursorPos.y +
		...
		float stretch_w =
			ImMax(0.0f, GetContentRegionAvail().x - min_w);
		pressed = Selectable("", false,
			selectable_flags | ImGuiSelectableFlags_SpanAvailWidth,
			ImVec2(min_w, label_size.y));
		if (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible) {
			RenderText(pos + ImVec2(offsets->OffsetLabel, 0.0f),
				label);
			if (icon_w > 0.0f)
				RenderText(
					pos + ImVec2(offsets->OffsetIcon, 0.0f),
					icon);
			if (shortcut_w > 0.0f) {
				PushStyleColor(ImGuiCol_Text,
...
bool ImGui::BeginTabBar(const char *str_id, ImGuiTabBarFlags flags)
{
	...
	ImRect tab_bar_bb = ImRect(window->DC.CursorPos.x,
		window->DC.CursorPos.y, window->WorkRect.Max.x,
...
// not scrolling)
static float TabBarCalcScrollableWidth(
	ImGuiTabBar *tab_bar, ImGuiTabBarSection *sections)
{
...
static void ImGui::TabBarLayout(ImGuiTabBar *tab_bar)
{
	...
	for (int tab_src_n = 0; tab_src_n < tab_bar->Tabs.Size; tab_src_n++) {
		...
		// We will need sorting if tabs have changed section (e.g. moved from one
		// of Leading/Central/Trailing to another)
	...
	for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++) {
		...
		// Additionally, when using TabBarAddTab() to manipulate tab bar order we
		// occasionally insert new tabs that don't have a width yet, and we
		...
		tab->ContentWidth = (tab->RequestedWidth >= 0.0f)
			? tab->RequestedWidth
			: TabItemCalcSize(
				  tab_name, has_close_button_or_unsaved_marker)
				  .x;
	...
	if ((tab_bar->WidthAllTabsIdeal > tab_bar->BarRect.GetWidth() &&
		...
		if (ImGuiTabItem *scroll_and_select_tab =
			...
			if ((scroll_and_select_tab->Flags &
				    ImGuiTabItemFlags_Button) == 0)
	...
	if (width_excess >= 1.0f &&
		...
		for (int tab_n = shrink_data_offset;
			...
			tab_n++) {
			ImGuiTabItem *tab =
				&tab_bar->Tabs[g.ShrinkWidthBuffer[tab_n]
						       .Index];
	...
	if (scroll_to_tab_id != 0)
		...
		const float wheel = g.IO.MouseWheelRequestAxisSwap
			? g.IO.MouseWheel
			: g.IO.MouseWheelH;
		...
		if (TestKeyOwner(wheel_key, tab_bar->ID) && wheel != 0.0f) {
			const float scroll_step = wheel *
				TabBarCalcScrollableWidth(tab_bar, sections) /
				3.0f;
	...
	tab_bar->ScrollingRectMinX = tab_bar->BarRect.Min.x +
		sections[0].Width + sections[0].Spacing;
	tab_bar->ScrollingRectMaxX = tab_bar->BarRect.Max.x -
		sections[2].Width - sections[1].Spacing;
...
// Dockable windows uses Name/ID in the global namespace. Non-dockable items
// use the ID stack.
	...
	ImGuiTabBar *tab_bar, ImGuiID tab_id, ImGuiTabBarSection *sections)
{
	...
	if (tab_bar->ScrollingTarget > tab_x1 ||
		...
		tab_bar->ScrollingTargetDistToVisibility = ImMax(
			(tab_x1 - scrollable_width) - tab_bar->ScrollingAnim,
			0.0f);
...
static ImGuiTabItem *ImGui::TabBarScrollingButtons(ImGuiTabBar *tab_bar)
{
	...
	if (select_dir != 0)
		if (ImGuiTabItem *tab_item = TabBarFindTabByID(
			...
			int selected_order =
				TabBarGetTabOrder(tab_bar, tab_item);
			...
			while (tab_to_scroll_to == NULL) {
				...
				// (even if first/last item is a button, return it so we can update
				// the scroll)
	...
	const char *label, bool *p_open, ImGuiTabItemFlags flags)
{
	...
	if (tab_bar == NULL) {
		IM_ASSERT_USER_ERROR(tab_bar,
			"Needs to be called between BeginTabBar() and "
			"EndTabBar()!");
		return false;
	}
	...
	ImGuiTabItemFlags flags, ImGuiWindow *docked_window)
{
	...
	ImVec2 size = TabItemCalcSize(label,
		(p_open != NULL) ||
	...
	if (is_central_section)
		window->DC.CursorPos = tab_bar->BarRect.Min +
			ImVec2(IM_TRUNC(tab->Offset - tab_bar->ScrollingAnim),
				0.0f);
	else
		window->DC.CursorPos =
	...
	if (held && !tab_appearing && IsMouseDragging(0)) {
		if (!g.DragDropActive &&
			(tab_bar->Flags & ImGuiTabBarFlags_Reorderable)) {
			// While moving a tab it will jump on the other side of the mouse, so
	...
	if (is_visible) {
		...
		if (tab_contents_visible &&
			...
			ImVec2 tl = bb.GetTL() +
				ImVec2(0, 1.0f * g.CurrentDpiScale);
			ImVec2 tr = bb.GetTR() +
				ImVec2(0, 1.0f * g.CurrentDpiScale);
		...
		// up with HoveredIdTimer-> seems ok) (We test IsItemHovered() to discard
		// e.g. when another item is active or drag and drop over the tab bar,
		// which g.HoveredId ignores)
...
// one-frame glitches when a tab has been unexpectedly removed. To use it to
// need to call the function SetTabItemClosed() between BeginTabBar() and
// EndTabBar(). Tabs closed by the close button will automatically be flagged
// to avoid this issue.
	...
	const char *label, bool has_close_button_or_unsaved_marker)
{
	...
	if (has_close_button_or_unsaved_marker)
		size.x += g.Style.FramePadding.x +
			(g.Style.ItemInnerSpacing.x +
				g.FontSize); // We use Y intentionally to fit
	...
	bool *out_just_closed, bool *out_text_clipped)
{
	...
	ImRect text_ellipsis_clip_bb(bb.Min.x + frame_padding.x,
		bb.Min.y + frame_padding.y, bb.Max.x - frame_padding.x,
		bb.Max.y);

	// Return clipped state ignoring the close button
	if (out_text_clipped) {
		*out_text_clipped =
			(text_ellipsis_clip_bb.Min.x + label_size.x) >
	...
	bool is_hovered = g.HoveredId == tab_id ||
		g.HoveredId == close_button_id || g.ActiveId == tab_id ||
		g.ActiveId ==
	...
	if (unsaved_marker_visible) {
		const ImRect bullet_bb(
			button_pos, button_pos + ImVec2(button_sz, button_sz));
