...
//-----------------------------------------------------------------------------
// A) You may edit imconfig.h (and not overwrite it when updating Dear ImGui,
// or maintain a patch/rebased branch with your modifications to it) B) or
// '#define IMGUI_USER_CONFIG "my_imgui_config.h"' in your project and then add
...
// that the data structures your files are using are matching the ones
// imgui.cpp is using.
//-----------------------------------------------------------------------------
...
// tool during development. They are extremely useful in day to day work.
...
// IMGUI_DISABLE_DEBUG_TOOLS                         // Disable
...
// kernel32.lib/.a) #define IMGUI_ENABLE_WIN32_DEFAULT_IME_FUNCTIONS // [Win32]
// [Default with Visual Studio] Implement default IME handler (require
// imm32.lib/.a, auto-link for Visual Studio, -limm32 on command-line for
// MinGW) #define IMGUI_DISABLE_WIN32_DEFAULT_IME_FUNCTIONS         // [Win32]
// [Default with non-Visual Studio compilers] Don't implement default IME
// handler (won't require imm32.lib/.a) #define IMGUI_DISABLE_WIN32_FUNCTIONS
...
// IMGUI_DISABLE_DEFAULT_FONT                        // Disable default
// embedded font (ProggyClean.ttf), remove ~9.5 KB from output binary.
// AddFontDefault() will assert. #define IMGUI_DISABLE_SSE // Disable use of
...
// available outside of Dear ImGui sources files.
...
//---- Use stb_sprintf.h for a faster implementation of vsnprintf instead of
// the one from libc (unless IMGUI_DISABLE_DEFAULT_FORMAT_FUNCTIONS is defined)
...
//---- Use FreeType + plutosvg or lunasvg to render OpenType SVG fonts
//(SVGinOT)
...
// Your renderer backend will need to support it (most example renderer
// backends support both 16/32-bit indices). Another way to allow large meshes
