	...
	ImGuiTableFlags flags, const ImVec2 &outer_size, float inner_width)
{
	...
	// users, but could make outside access broken or confusing, e.g. TestEngine.
	...
	const ImVec2 actual_outer_size =
		ImTrunc(CalcItemSize(outer_size, ImMax(avail_size.x, 1.0f),
	...
	if (instance_no > 0) {
		...
		if (table->InstanceDataExtra.Size < instance_no)
			table->InstanceDataExtra.push_back(
	...
	if (use_child_window) {
		...
		if (table->InnerWindow->SkipItems &&
			outer_window_is_measuring_size)
		...
		// EndTable().
		table->WorkRect = table->OuterRect = table->InnerRect =
			outer_rect;
	...
	// commands. However since the overlap problem only affect scrolling tables
	// in this case we can get away with doing it without extra cost).
	if (inner_window != outer_window) {
		...
		if (flags & ImGuiTableFlags_BordersOuterV) {
			...
			if (inner_window->DecoOuterSizeX2 == 0.0f)
				table->HostClipRect.Max.x =
					ImMax(table->HostClipRect.Max.x -
							TABLE_BORDER_SIZE,
		...
		if (flags & ImGuiTableFlags_BordersOuterH) {
			...
			if (inner_window->DecoOuterSizeY2 == 0.0f)
				table->HostClipRect.Max.y =
					ImMax(table->HostClipRect.Max.y -
							TABLE_BORDER_SIZE,
	...
	const float inner_spacing_for_border =
		(flags & ImGuiTableFlags_BordersInnerV) ? TABLE_BORDER_SIZE
							: 0.0f;
	...
	const float inner_padding_explicit =
		...
		: 0.0f;
	table->CellSpacingX1 =
		inner_spacing_explicit + inner_spacing_for_border;
	table->CellSpacingX2 = inner_spacing_explicit;
	table->CellPaddingX = inner_padding_explicit;

	const float outer_padding_for_border =
		(flags & ImGuiTableFlags_BordersOuterV) ? TABLE_BORDER_SIZE
							: 0.0f;
	...
	if (inner_window !=
		outer_window) // So EndChild() within the inner window
	...
	if (table->IsInitializing) {
		...
		table->IsSettingsDirty =
			true; // Records itself into .ini file even when
			      // in default state (#7934)
		table->InstanceInteracted = -1;
		table->ContextPopupColumn = -1;
		table->ReorderColumn = table->ResizedColumn =
...
void ImGui::TableBeginInitMemory(ImGuiTable *table, int columns_count)
{
	...
	span_allocator.Reserve(
		2, columns_count * sizeof(ImGuiTableCellData), 4);
	for (int n = 3; n < 6; n++)
	...
	if (table->InstanceCurrent == 0) {
		...
		if (table->AutoFitSingleColumn != -1) {
			TableSetColumnWidth(table->AutoFitSingleColumn,
				table->Columns[table->AutoFitSingleColumn]
					.WidthAuto);
			table->AutoFitSingleColumn = -1;
		}
	}
...
void ImGui::TableUpdateLayout(ImGuiTable *table)
{
	...
	ImBitArrayClearAllBits(
		table->EnabledMaskByDisplayOrder, table->ColumnsCount);
	...
	for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
		...
		if (table->DeclColumnsCount <= column_n) {
			TableSetupColumnFlags(
				table, column, ImGuiTableColumnFlags_None);
			column->NameOffset = -1;
			column->UserID = 0;
		...
		if (prev_visible_column_idx != -1)
			table->Columns[prev_visible_column_idx]
				.NextEnabledColumn =
				(ImGuiTableColumnIdx)column_n;
		else
		...
		IM_ASSERT(
			column->IndexWithinEnabledSet <= column->DisplayOrder);
	...
	for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
		...
		if (column->Flags & ImGuiTableColumnFlags_WidthStretch) {
			float weight_ratio =
				column->StretchWeight / stretch_sum_weights;
		...
		column->WidthGiven = ImTrunc(
			ImMax(column->WidthRequest, table->MinColumnWidth));
	...
	if (width_remaining_for_stretched_columns >= 1.0f &&
		...
		for (int order_n = table->ColumnsCount - 1;
			...
			order_n >= 0;
			order_n--) {
			if (!IM_BITARRAY_TESTBIT(
				    table->EnabledMaskByDisplayOrder, order_n))
				continue;
	...
	const ImRect mouse_hit_rect(table->OuterRect.Min.x,
		table->OuterRect.Min.y, table->OuterRect.Max.x,
	...
	for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
		...
		column->WidthGiven =
			ImMin(column->WidthGiven, column->WidthMax);
	...
	if (has_at_least_one_column_requesting_output == false) {
		table->Columns[table->LeftMostEnabledColumn].IsRequestOutput =
			true;
		table->Columns[table->LeftMostEnabledColumn].IsSkipItems =
			false;
	}
...
void ImGui::TableUpdateBorders(ImGuiTable *table)
{
	...
	ImGuiTableInstanceData *table_instance =
		TableGetInstanceData(table, table->InstanceCurrent);
	const float hit_half_width = ImTrunc(
	...
	const float hit_y2_body = ImMax(table->OuterRect.Max.y,
		hit_y1 + table_instance->LastOuterHeight -
			table->AngledHeadersHeight);
	const float hit_y2_head =
		hit_y1 + table_instance->LastTopHeadersRowHeight;
		...
		const float new_x2 =
			(g.IO.MousePos.x - g.ActiveIdClickOffset.x +
				ImTrunc(TABLE_RESIZE_SEPARATOR_HALF_THICKNESS *
					g.CurrentDpiScale));
		const float new_width = ImTrunc(new_x2 - column->MinX -
...
// Called in TableSetupColumn() when initializing and in TableLoadSettings()
// for defaults before applying stored settings. 'init_mask' specify which
	...
	ImGuiTableColumnFlags init_mask)
{
	...
	if (init_mask & ImGuiTableFlags_Resizable) {
		...
		if (init_width_or_weight >
			0.0f) // Disable auto-fit if an explicit
			      // width/weight has been specified
			column->AutoFitQueue = 0x00;
	}
	if (init_mask & ImGuiTableFlags_Reorderable)
		column->DisplayOrder =
			(ImGuiTableColumnIdx)table->Columns.index_from_ptr(
				column);
	...
	float init_width_or_weight, ImGuiID user_id)
{
	...
	if (table == NULL) {
		IM_ASSERT_USER_ERROR(table != NULL,
			"Call should only be done while in BeginTable() "
			"scope!");
		return;
	}
	IM_ASSERT(table->IsLayoutLocked == false &&
...
void ImGui::TableSetupScrollFreeze(int columns, int rows)
{
	...
	for (int column_n = 0; column_n < table->FreezeColumnsRequest;
		...
		if (order_n != column_n &&
			order_n >= table->FreezeColumnsRequest) {
			ImSwap(table->Columns
					[table->DisplayOrderToIndex[order_n]]
						.DisplayOrder,
				table->Columns
...
const char *ImGui::TableGetColumnName(const ImGuiTable *table, int column_n)
{
	if (table->IsLayoutLocked == false &&
		column_n >= table->DeclColumnsCount)
...
void ImGui::TableSetColumnEnabled(int column_n, bool enabled)
{
	...
	IM_ASSERT(
		table->Flags & ImGuiTableFlags_Hideable); // See comments above
	if (column_n < 0)
		column_n = table->CurrentColumn;
...
void ImGui::TableEndRow(ImGuiTable *table)
{
	...
	if (is_visible) {
		...
		if (table->HoveredColumnBody != -1 &&
			g.IO.MousePos.y >= bg_y1 && g.IO.MousePos.y < bg_y2 &&
		...
		if (table->CurrentRow > 0 &&
			...
			top_border_col = (table->LastRowFlags &
	...
	if (unfreeze_rows_request) {
		for (int column_n = 0; column_n < table->ColumnsCount;
			column_n++)
			table->Columns[column_n].NavLayerCurrent =
				table->NavLayer;
		const float y0 =
		...
		if (unfreeze_rows_actual) {
			...
			SetWindowClipRectBeforeSetChannel(
				window, table->Columns[0].ClipRect);
...
void ImGui::TablePopBackgroundChannel()
{
	...
	SetWindowClipRectBeforeSetChannel(
		window, table->HostBackupInnerClipRect);
	table->DrawSplitter->SetCurrentChannel(window->DrawList,
...
void ImGui::TableSetupDrawChannels(ImGuiTable *table)
{
	...
	table->DrawSplitter->Split(
		table->InnerWindow->DrawList, channels_total);
...
void ImGui::TableMergeDrawChannels(ImGuiTable *table)
{
	...
	for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
		...
		for (int merge_group_sub_n = 0;
			...
			const int merge_group_n =
				(has_freeze_h && column_n < table->FreezeColumnsCount
						? 0
						: 1) +
				(has_freeze_v && merge_group_sub_n == 0 ? 0
									: 2);
	...
	if (merge_group_mask != 0) {
		...
		ImBitArraySetBitRange(remaining_mask, LEADING_DRAW_CHANNELS,
			splitter->_Count);
		ImBitArrayClearBit(
			remaining_mask, table->Bg2DrawChannelUnfrozen);
		...
		for (int merge_group_n = 0;
			...
			if (int merge_channels_count =
				...
				for (int n = 0;
					n < (size_for_masks_bitarrays_one >> 2);
					n++)
					remaining_mask[n] &=
						~merge_group->ChannelsMask[n];
...
static ImU32 TableGetColumnBorderCol(
	ImGuiTable *table, int order_n, int column_n)
{
	...
	if (table->Flags & ImGuiTableFlags_BordersOuter) {
		...
		if ((table->Flags & ImGuiTableFlags_BordersOuter) ==
			...
			inner_drawlist->AddRect(outer_border.Min,
				outer_border.Max, outer_col, 0.0f, 0,
				border_size);
...
{
	if (column->SortOrder == -1 ||
		(column->SortDirectionsAvailMask &
	...
	if (sort_order_count == 0 &&
		...
		for (int column_n = 0; column_n < table->ColumnsCount;
			...
			if (column->IsEnabled &&
				...
				column->SortOrder = 0;
				column->SortDirection =
					(ImU8)TableGetColumnAvailSortDirection(
						column, 0);
				break;
			}
		}
...
void ImGui::TableSortSpecsBuild(ImGuiTable *table)
{
	...
	if (dirty && sort_specs != NULL)
		for (int column_n = 0; column_n < table->ColumnsCount;
			...
			sort_spec->SortOrder =
				(ImGuiTableColumnIdx)column->SortOrder;
		...
		if (IM_BITARRAY_TESTBIT(table->EnabledMaskByIndex, column_n))
			if ((table->Columns[column_n].Flags &
				...
				row_height = ImMax(row_height,
					CalcTextSize(TableGetColumnName(
							     table, column_n))
						.y);
...
// intentionally written to not make much use of internal functions, to give
// you better direction if you need to write your own.
...
void ImGui::TableHeader(const char *label)
{
	...
	float label_height = ImMax(label_size.y,
	...
	if ((table->Flags & ImGuiTableFlags_Sortable) &&
		...
		w_arrow = ImTrunc(
			g.FontSize * ARROW_SCALE + g.Style.FramePadding.x);
		if (column->SortOrder != -1)
			sort_arrow = true;
		if (column->SortOrder > 0) {
	...
	ItemSize(ImVec2(
		0.0f, label_height)); // Don't declare unclipped width, it'll
	...
	bool pressed = ButtonBehavior(
		bb, id, &hovered, &held, ImGuiButtonFlags_AllowOverlap);
	if (held || hovered || highlight) {
		...
		TableSetBgColor(
			ImGuiTableBgTarget_CellBg, col, table->CurrentColumn);
	} else {
	...
	if ((table->Flags & ImGuiTableFlags_Sortable) &&
		...
		if (column->SortOrder != -1) {
			...
			if (column->SortOrder > 0) {
				...
				RenderText(
					ImVec2(x + g.Style.ItemInnerSpacing.x,
						y),
					sort_order_suf);
				PopStyleColor();
				x += w_sort_text;
	...
	RenderTextEllipsis(window->DrawList, label_pos,
		ImVec2(ellipsis_max, bb.Max.y), ellipsis_max, label, label_end,
		&label_size);
	...
	for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
		if (IM_BITARRAY_TESTBIT(
			...
			ImGuiTableHeaderData request = {
				(ImGuiTableColumnIdx)column_n, col_text,
				col_header_bg,
...
void ImGui::TableAngledHeadersRowEx(ImGuiID row_id, float angle,
	float max_label_width, const ImGuiTableHeaderData *data, int data_count)
{
	ImGuiContext &g = *GImGui;
	ImGuiTable *table = g.CurrentTable;
	ImGuiWindow *window = g.CurrentWindow;
	...
	const ImVec2 header_angled_vector = unit_right *
		(row_height /
			-sin_a); // vector from bottom-left to top-left, and
	...
	table->DrawSplitter->SetCurrentChannel(
		draw_list, TABLE_DRAW_CHANNEL_BG0);
		...
		for (int order_n = 0; order_n < data_count; order_n++) {
			...
			if (pass == 0) {
				...
				draw_list->AddQuadFilled(bg_shape[0],
					bg_shape[1], bg_shape[2], bg_shape[3],
					request->BgColor0);
				draw_list->AddQuadFilled(bg_shape[0],
					bg_shape[1], bg_shape[2], bg_shape[3],
				...
				// Draw label
				// - First draw at an offset where RenderTextXXX() function won't
				// meddle with applying current ClipRect, then transform to final
				// offset.
				// - Handle multiple lines manually, as we want each lines to
				// follow on the horizontal border, rather than see a whole block
				// rotated.
				const char *label_name =
					TableGetColumnName(table, column_n);
				const char *label_name_end =
					FindRenderedTextEnd(label_name);
				const float line_off_step_x =
					(g.FontSize / -sin_a);
				const int label_lines = ImTextCountLines(
					label_name, label_name_end);

				// Left<>Right alignment
				float line_off_curr_x = flip_label
					? (label_lines - 1) * line_off_step_x
					: 0.0f;
				float line_off_for_align_x =
					ImMax((((column->MaxX - column->MinX) -
						...
						0.0f) *
					align.x;
				line_off_curr_x += line_off_for_align_x -
					line_off_for_ascent_x;
				...
				while (label_name < label_name_end) {
					const char *label_name_eol =
						strchr(label_name, '\n');
					if (label_name_eol == NULL)
						label_name_eol = label_name_end;

					// FIXME: InDiviseurdual line clipping for right-most column is
					// broken for negative angles.
					ImVec2 label_size = CalcTextSize(
					...
					const float vertical_offset =
						available_space * align.y *
						(flip_label ? -1.0f : 1.0f);

					// Rotate and offset label
					ImVec2 pivot_in =
						ImVec2(window->ClipRect.Min.x -
								vertical_offset,
							window->ClipRect.Min.y +
								label_size.y);
					ImVec2 pivot_out = ImVec2(
						column->WorkMinX, row_r.Max.y);
					line_off_curr_x += flip_label
						? -line_off_step_x
						: line_off_step_x;
					...
					ShadeVertsTransformPos(draw_list,
						vtx_idx_begin, vtx_idx_end,
						pivot_in, label_cos_a,
						label_sin_a,
					...
					// fg_dl->_VtxCurrentIdx, pivot_in, label_cos_a, label_sin_a,
					// pivot_out); }
	...
	PopClipRect();
	PopClipRect();
	table->TempData->AngledHeadersExtraWidth = ImMax(0.0f,
	...
	ImGuiTable *table, ImGuiTableFlags flags_for_section_to_display)
{
	...
	if (flags_for_section_to_display & ImGuiTableFlags_Hideable) {
		...
		for (int other_column_n = 0;
			other_column_n < table->ColumnsCount;
			other_column_n++) {
			ImGuiTableColumn *other_column =
				&table->Columns[other_column_n];
			if (other_column->Flags &
...
void ImGui::TableSaveSettings(ImGuiTable *table)
{
	...
	for (int n = 0; n < table->ColumnsCount;
		...
		if (column->IsUserEnabled !=
			((column->Flags & ImGuiTableColumnFlags_DefaultHide) ==
				0))
...
void ImGui::TableGcCompactTransientBuffers(ImGuiTable *table)
{
	...
	table->IsSortSpecsDirty =
		true; // FIXME: In theory shouldn't have to leak
...
void ImGui::DebugNodeTable(ImGuiTable *table)
{
	...
	bool open = TreeNode(table, "Table 0x%08X (%d columns, in '%s')%s",
	...
	if (IsItemVisible() && table->HoveredColumnBody != -1)
		GetForegroundDrawList()->AddRect(GetItemRectMin(),
			GetItemRectMax(), IM_COL32(255, 255, 0, 255));
	if (!open)
		return;
	if (table->InstanceCurrent > 0)
		Text("** %d instances of same table! Some data below will "
		     "refer to last "
		     "instance.",
			table->InstanceCurrent + 1);
	...
	BulletText("CellPaddingX: %.1f, CellSpacingX: %.1f/%.1f, "
		   "OuterPaddingX: %.1f",
	...
	for (int n = 0; n < table->InstanceCurrent + 1; n++) {
		...
		BulletText("Instance %d: HoveredRow: %d, LastOuterHeight: %.2f",
			n, table_instance->HoveredRowLast,
			table_instance->LastOuterHeight);
	}
	...
	for (int n = 0; n < table->ColumnsCount; n++) {
		...
		ImFormatString(buf, IM_ARRAYSIZE(buf),
			...
			"StretchWeight: %.3f "
			"(%.1f%%)\n"
			"MinX: %.1f, MaxX: %.1f (%+.1f), ClipRect: %.1f to "
			"%.1f (+%.1f)\n"
			...
			column->IsEnabled, column->IsVisibleX,
			column->IsVisibleY, column->IsRequestOutput,
...
void ImGui::DebugNodeTableSettings(ImGuiTableSettings *settings)
{
	...
	for (int n = 0; n < settings->ColumnsCount; n++) {
		...
		BulletText("Column %d Order %d SortOrder %d %s Vis %d %s %7.3f "
			   "UserID 0x%08X",
	...
	ImGuiOldColumns *columns, int column_index, bool before_resize = false)
{
	...
	float offset_norm;
	if (before_resize)
		offset_norm = columns->Columns[column_index + 1]
...
void ImGui::SetColumnWidth(int column_index, float width)
{
	...
	if (column_index < 0)
		column_index = columns->Current;
	SetColumnOffset(
		column_index + 1, GetColumnOffset(column_index) + width);
}
...
void ImGui::PopColumnsBackground()
{
	...
	columns->Splitter.SetCurrentChannel(
		window->DrawList, columns->Current + 1);
}
	...
	const char *str_id, int columns_count, ImGuiOldColumnFlags flags)
{
	...
	const float half_clip_extend_x = ImTrunc(ImMax(
	...
	columns->OffMinX = window->DC.Indent.x - column_padding +
		ImMax(column_padding - window->WindowPadding.x, 0.0f);
	columns->OffMaxX = ImMax(
		ImMin(max_1, max_2) - window->Pos.x, columns->OffMinX + 1.0f);
...
void ImGui::NextColumn()
{
	...
	if (columns->Count == 1) {
		window->DC.CursorPos.x = IM_TRUNC(window->Pos.x +
	...
	if (!(flags & ImGuiOldColumnFlags_NoBorder) && !window->SkipItems) {
		...
		for (int n = 1; n < columns->Count; n++) {
			...
			const float column_hit_hw =
				ImTrunc(COLUMNS_HIT_RECT_HALF_THICKNESS *
					g.CurrentDpiScale);
...
void ImGui::Columns(int columns_count, const char *id, bool borders)
{
	...
	ImGuiOldColumnFlags flags =
		(borders ? 0 : ImGuiOldColumnFlags_NoBorder);
