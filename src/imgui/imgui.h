...
// - Add '#define IMGUI_DEFINE_MATH_OPERATORS' before including this file (or
// in imconfig.h) to access courtesy maths operators for ImVec2 and ImVec4.
...
// Library Version
// (Integer encoded as XYYZZ for use in #if preprocessor conditionals, e.g.
// '#if IMGUI_VERSION_NUM >= 12345')
...
// (edit imconfig.h or '#define IMGUI_USER_CONFIG "myfilename.h" from your
// build system)
...
#define IM_ARRAYSIZE(_ARR)                                                     \
	((int)(sizeof(_ARR) /                                                  \
		sizeof(*(                                                      \
			_ARR)))) // Size of a static C-style array.           \
...
// imgui code and caller. Read comments above DebugCheckVersionAndDataLayout()
// for details.
...
//  location. e.g. #include <sal.h> + void myprintf(_Printf_format_string_
//  const char* format, ...))
...
//   (e.g. Used by DX11 backend to a `ID3D11ShaderResourceView*`; Used by
//   OpenGL backends to store `GLuint`;
...
// build on 32-bit architectures. Use intermediary intptr_t cast and read FAQ
// if you have casting warnings.
// - In v1.92.0 (2025/XX/XX): added ImTextureRef which carry either a
...
//   to extract the ImTextureID value during rendering, after texture upload
//   has happened.
// - There is no constructor to create a ImTextureID from a ImTextureData* as
...
// use io.Fonts->TexRef.
// - Binding generators for languages such as C (which don't have
// constructors), should provide a helper, e.g.
...
struct ImTextureRef {
	...
	inline ImTextureID GetTexID()
		const; // == (_TexData ? _TexData->TexID : _TexID) // Implemented
		// below in the file.
...
IMGUI_API void ShowFontSelector(const char
		*label); // add font selector block (not a window), essentially
			 // a combo listing the loaded fonts.
IMGUI_API void
ShowUserGuide(); // add basic help/info block (not a window): how to
		 // manipulate ImGui as an end-user (mouse/keyboard controls).
...
IMGUI_API void SetWindowSize(const char *name, const ImVec2 &size,
	ImGuiCond cond = 0); // set named window size. set axis to 0.0f
...
//  - PushFont(font, 0.0f)                       // Change font and keep
//  current size
//  - PushFont(NULL, 20.0f)                      // Keep font and change
//  current size
...
// *IMPORTANT* before 1.92, fonts had a single size. They can now be
// dynamically be adjusted.
//  - In 1.92 we have REMOVED the single parameter version of PushFont()
//  because it seems like the easiest way to provide an error-proof transition.
...
// almost only ever useful to convert from window-local to absolute
...
IMGUI_API void SetCursorScreenPos(
	const ImVec2 &pos); // cursor position, absolute coordinates.
...
IMGUI_API bool TextLinkOpenURL(const char *label,
...
// kept available for convenience purpose. This is analogous to how ListBox are
// created.
...
// function, note that a 'float v[X]' function argument is the same as 'float*
// v',
...
// "%d").
// - Speed are per-pixel of mouse movement (v_speed=0.2f: mouse needs to move
// by 5 pixels to increase value by 1). For keyboard/gamepad navigation,
...
// purpose. This is analogous to how Combos are created.
...
//  pressing ESCAPE.
//  - Their visibility state (~bool) is held internally instead of being held
//  by the programmer as we are used to with regular Begin*() calls.
...
IMGUI_API bool BeginPopupContextVoid(const char *str_id = NULL,
	ImGuiPopupFlags popup_flags =
		1); // open+begin popup when clicked in void
...
//  - IsPopupOpen() with ImGuiPopupFlags_AnyPopupId: return true if any popup
//  is open at the current BeginPopup() level of the popup stack.
...
//   When 'sort_specs->SpecsDirty == true' you should sort your data. It will
//   be true when sorting specs have changed since last call, or the first
...
IMGUI_API void Columns(
	int count = 1, const char *id = NULL, bool borders = true);
...
// provided to facilitate use of boolean expressions (as a micro-optimization:
// if you have tens of thousands of BeginDisabled(false)/EndDisabled() pairs,
...
IMGUI_API void SetNavCursorVisible(
	bool visible); // alter visibility of keyboard/gamepad
...
SetNextItemAllowOverlap(); // allow next item to be overlapped by a subsequent
			   ...
			   // may need to be added. Note that both
			   // Selectable() and TreeNode() have dedicated flags
			   // doing this.
...
IMGUI_API int GetFrameCount(); // get global imgui frame count. incremented by
			       // 1 every frame.
...
IMGUI_API const char *GetStyleColorName(
	ImGuiCol idx); // get a string corresponding to the enum
		       // value (for display, saving, etc.).
IMGUI_API void SetStateStorage(ImGuiStorage
		*storage); // replace current window storage with our own (if
...
IMGUI_API void SetNextItemShortcut(
...
IsAnyMouseDown(); // [WILL OBSOLETE] is any mouse button held? This was
		  ...
		  // a mask of held mouse buttons, because upcoming input
...
GetMousePosOnOpeningCurrentPopup(); // retrieve mouse position at the time of
	...
	// (helper to avoid user backing that
	// value themselves)
...
enum ImGuiWindowFlags_ {
	...
	ImGuiWindowFlags_NoNavInputs = 1
		<< 16, // No keyboard/gamepad navigation within the window
	ImGuiWindowFlags_NoNavFocus = 1
		<< 17, // No focusing toward this window with keyboard/gamepad
...
// Obsolete names
	...
	ImGuiWindowFlags_NavFlattened = 1
		<< 29, // Obsoleted in 1.90.9: Use ImGuiChildFlags_NavFlattened in
	// BeginChild() call.
...
//   - You may also use ImGuiChildFlags_AlwaysAutoResize to force an update
//   even when child window is not in view.
...
enum ImGuiChildFlags_ {
	...
	ImGuiChildFlags_Borders = 1
		<< 0, // Show an outer border and enable WindowPadding. (IMPORTANT: this
		      // is always == 1 == true for legacy reason)
	ImGuiChildFlags_AlwaysUseWindowPadding = 1
		<< 1, // Pad with style.WindowPadding even if no border are drawn (no
	// padding by default for non-bordered child windows because it
	// makes more sense)
	ImGuiChildFlags_ResizeX = 1
		<< 2, // Allow resize from right border (layout direction). Enable .ini
	// saving (unless ImGuiWindowFlags_NoSavedSettings passed to
	// window flags)
	ImGuiChildFlags_ResizeY = 1
		<< 3, // Allow resize from bottom border (layout direction). "
	ImGuiChildFlags_AutoResizeX = 1
		<< 4, // Enable auto-resizing width. Read "IMPORTANT: Size measurement"
		      // details above.
	ImGuiChildFlags_AutoResizeY = 1
		<< 5, // Enable auto-resizing height. Read "IMPORTANT: Size measurement"
		      // details above.
	ImGuiChildFlags_AlwaysAutoResize = 1
		<< 6, // Combined with AutoResizeX/AutoResizeY. Always measure size even
	...
	ImGuiChildFlags_NavFlattened = 1
		<< 8, // [BETA] Share focus scope, allow keyboard/gamepad navigation to
...
enum ImGuiItemFlags_ {
	...
	ImGuiItemFlags_NoTabStop = 1
		<< 0, // false    // Disable keyboard tabbing. This is a "lighter"
		      // version of ImGuiItemFlags_NoNav.
	ImGuiItemFlags_NoNav = 1
		<< 1, // false    // Disable any form of focusing (keyboard/gamepad
	// directional navigation and SetKeyboardFocusHere() calls).
	ImGuiItemFlags_NoNavDefaultFocus = 1
		<< 2, // false    // Disable item being a candidate for default focus
		      // (e.g. used by title bar items).
	ImGuiItemFlags_ButtonRepeat = 1
		<< 3, // false    // Any button-like behavior will have repeat mode
	...
	ImGuiItemFlags_AutoClosePopups = 1
		<< 4, // true     // MenuItem()/Selectable() automatically close their
		      // parent popup window.
...
enum ImGuiInputTextFlags_ {
	...
	ImGuiInputTextFlags_EscapeClearsAll = 1
		<< 7, // Escape key clears content if not empty, and deactivate
	// otherwise (contrast to default behavior of Escape to revert)
	ImGuiInputTextFlags_CtrlEnterForNewLine = 1
		<< 8, // In multi-line mode, validate with Enter, add new line with
	...
	ImGuiInputTextFlags_Password = 1
		<< 10, // Password mode, display all characters as '*', disable copy
	...
	ImGuiInputTextFlags_NoHorizontalScroll = 1
		<< 15, // Disable following the cursor horizontally
	...
	ImGuiInputTextFlags_ElideLeft = 1
		<< 17, // When text doesn't fit, elide left side to ensure right side
	// stays visible. Useful for path/filenames. Single-line only!

	// Callback features
	ImGuiInputTextFlags_CallbackCompletion = 1
		<< 18, // Callback on pressing TAB (for completion handling)
	ImGuiInputTextFlags_CallbackHistory = 1
		<< 19, // Callback on pressing Up/Down arrows (for history handling)
	ImGuiInputTextFlags_CallbackAlways = 1
		<< 20, // Callback on each iteration. User code may query cursor
	// position, modify text buffer.
	ImGuiInputTextFlags_CallbackCharFilter = 1
		<< 21, // Callback on character inputs to replace or discard them.
	...
	ImGuiInputTextFlags_CallbackEdit = 1
		<< 23, // Callback on any edit. Note that InputText() already returns
...
enum ImGuiTreeNodeFlags_ {
	...
	ImGuiTreeNodeFlags_AllowOverlap = 1
	...
	ImGuiTreeNodeFlags_OpenOnDoubleClick = 1
		<< 6, // Open on double-click instead of simple click (default for
	...
	ImGuiTreeNodeFlags_OpenOnArrow = 1
		<< 7, // Open when clicking on the arrow part (default for multi-select
	...
	ImGuiTreeNodeFlags_Bullet = 1
		<< 9, // Display a bullet instead of arrow. IMPORTANT: node can still be
	...
	ImGuiTreeNodeFlags_SpanFullWidth = 1
		<< 12, // Extend hit box to the left-most and right-most edges (cover
	// the indent area).
	ImGuiTreeNodeFlags_SpanLabelWidth = 1
		<< 13, // Narrow hit box + narrow hovering highlight, will only cover
	// the label text.
	ImGuiTreeNodeFlags_SpanAllColumns = 1
		<< 14, // Frame will span all columns of its container table (label will
	// still fit in current column)
	ImGuiTreeNodeFlags_LabelSpanAllColumns = 1
		<< 15, // Label will span all columns of its container table
	...
	ImGuiTreeNodeFlags_DrawLinesToNodes = 1
		<< 20, // Horizontal lines to child nodes. Vertical line drawn down to
...
enum ImGuiPopupFlags_ {
	...
	ImGuiPopupFlags_NoOpenOverItems = 1
		<< 8, // For BeginPopupContextWindow(): don't return true when hovering
	...
	ImGuiPopupFlags_AnyPopupLevel = 1
		<< 11, // For IsPopupOpen(): search/test at any level of the popup stack
...
enum ImGuiSelectableFlags_ {
	...
	ImGuiSelectableFlags_NoAutoClosePopups = 1
		<< 0, // Clicking this doesn't close parent popup window (overrides
		      // ImGuiItemFlags_AutoClosePopups)
	ImGuiSelectableFlags_SpanAllColumns = 1
		<< 1, // Frame will span all columns of its container table (text will
		      // still fit in current column)
	...
	ImGuiSelectableFlags_Highlight = 1
		<< 5, // Make the item be displayed as if it is hovered
...
enum ImGuiComboFlags_ {
	...
	ImGuiComboFlags_WidthFitPreview = 1
		<< 7, // Width dynamically calculated from preview contents
...
enum ImGuiTabBarFlags_ {
	...
	ImGuiTabBarFlags_NoCloseWithMiddleMouseButton = 1
		<< 3, // Disable behavior of closing tabs (that are submitted with p_open
	...
	ImGuiTabBarFlags_FittingPolicyResizeDown = 1
		<< 7, // Resize tabs when they don't fit
	ImGuiTabBarFlags_FittingPolicyScroll = 1
		<< 8, // Add scroll buttons when tabs don't fit
...
enum ImGuiTabItemFlags_ {
	...
	ImGuiTabItemFlags_UnsavedDocument = 1
		<< 0, // Display a dot next to the title + set
	...
	ImGuiTabItemFlags_NoCloseWithMiddleMouseButton = 1
		<< 2, // Disable behavior of closing tabs (that are submitted with p_open
	// != NULL) with middle mouse button. You may handle this behavior
	// manually on user's side with if (IsItemHovered() &&
	// IsMouseClicked(2)) *p_open = false.
	...
	ImGuiTabItemFlags_Trailing = 1
		<< 7, // Enforce the tab position to the right of the tab bar (before
		      // the scrolling buttons)
	ImGuiTabItemFlags_NoAssumedClosure = 1
...
enum ImGuiHoveredFlags_ {
	...
	ImGuiHoveredFlags_AllowWhenOverlappedByWindow = 1
		<< 9, // IsItemHovered() only: Return true even if the position is
	...
	ImGuiHoveredFlags_ForTooltip = 1
		<< 12, // Shortcut for standard flags when using IsItemHovered() +
	// SetTooltip() sequence.
	...
	ImGuiHoveredFlags_DelayNone = 1
		<< 14, // IsItemHovered() only: Return true immediately (default). As
	...
	ImGuiHoveredFlags_NoSharedDelay = 1
		<< 17, // IsItemHovered() only: Disable shared delay system where moving
...
enum ImGuiDragDropFlags_ {
	...
	ImGuiDragDropFlags_SourceNoPreviewTooltip = 1
		<< 0, // Disable preview tooltip. By default, a successful call to
	...
	ImGuiDragDropFlags_PayloadAutoExpire = 1
	...
	ImGuiDragDropFlags_PayloadNoCrossContext = 1
		<< 6, // Hint to specify that the payload may not be copied outside
		      // current dear imgui context.
	ImGuiDragDropFlags_PayloadNoCrossProcess = 1
		<< 7, // Hint to specify that the payload may not be copied outside
		      // current process.
...
// Keyboard, Mouse and Gamepad values. All our named keys are >= 512. Keys
// value 0 to 511 are left unused and were legacy native/opaque key values
// (< 1.87). Support for legacy keys was completely removed in 1.91.5. Read
// details about the 1.87+ transition :
...
enum ImGuiInputFlags_ {
	...
	ImGuiInputFlags_Repeat = 1
		<< 0, // Enable repeat. Return true on successive repeats. Default for
	...
	ImGuiInputFlags_RouteFocused = 1
		<< 11, // Route to windows in the focus stack (DEFAULT). Deep-most
	...
	ImGuiInputFlags_RouteAlways = 1
		<< 13, // Do not register route, poll keys directly.
	// - Routing options
	ImGuiInputFlags_RouteOverFocused = 1
		<< 14, // Option: global route: higher priority than focused route
	// (unless active item in focused route).
	ImGuiInputFlags_RouteOverActive = 1
	...
	ImGuiInputFlags_RouteFromRootWindow = 1
		<< 17, // Option: route evaluated from the point of view of root window
	// rather than current window.

	// Flags for SetNextItemShortcut()
	ImGuiInputFlags_Tooltip = 1
		<< 18, // Automatically display a tooltip when hovering item [BETA]
...
enum ImGuiConfigFlags_ {
	...
	ImGuiConfigFlags_NavEnableKeyboard = 1
		<< 0, // Master keyboard navigation enable flag. Enable full Tabbing +
		      // directional arrows + space/enter to activate.
	ImGuiConfigFlags_NavEnableGamepad = 1
		<< 1, // Master gamepad navigation enable flag. Backend also needs to
	...
	ImGuiConfigFlags_NoKeyboard = 1
		<< 6, // Instruct dear imgui to disable keyboard inputs and
...
enum ImGuiBackendFlags_ {
	...
	ImGuiBackendFlags_HasGamepad = 1
		<< 0, // Backend Platform supports gamepad
	...
	// texture updates and texture reloads.
};
...
enum ImGuiButtonFlags_ {
	...
	ImGuiButtonFlags_EnableNav = 1
		<< 3, // InvisibleButton(): do not disable navigation/tabbing. Otherwise
	// disabled by default.
};
...
enum ImGuiColorEditFlags_ {
	...
	ImGuiColorEditFlags_AlphaOpaque = 1
		<< 11, //              // ColorEdit, ColorPicker, ColorButton: disable
	...
	ImGuiColorEditFlags_AlphaNoBg = 1
		<< 12, //              // ColorEdit, ColorPicker, ColorButton: disable
	...
	// [Internal] Masks
	ImGuiColorEditFlags_AlphaMask_ = ImGuiColorEditFlags_NoAlpha |
...
enum ImGuiSliderFlags_ {
	...
	ImGuiSliderFlags_WrapAround = 1
		<< 8, // Enable wrapping around from max to min and from min to max.
	...
	ImGuiSliderFlags_ClampZeroRange = 1
...
// combine multiple values using binary operators! All the functions above
// treat 0 as a shortcut to ImGuiCond_Always.
enum ImGuiCond_ {
	ImGuiCond_None =
		0, // No condition (always set the variable), same as _Always
	ImGuiCond_Always = 1
		<< 0, // No condition (always set the variable), same as _None
	ImGuiCond_Once =
...
//      Fixed columns, followed by one or two TRAILING Stretch columns. (this
//      is because the visible order of columns have subtle but necessary
//      effects on how they react to manual resizing).
// - When ScrollX is on:
//    - Table defaults to ImGuiTableFlags_SizingFixedFit -> all Columns
...
enum ImGuiTableFlags_ {
	...
	ImGuiTableFlags_NoClip = 1
		<< 20, // Disable clipping rectangle for every inDiviseurdual columns
	...
	ImGuiTableFlags_PadOuterX = 1
		<< 21, // Default if BordersOuterV is on. Enable outermost padding.
	// Generally desirable if you have headers.
	ImGuiTableFlags_NoPadOuterX = 1
		<< 22, // Default if BordersOuterV is off. Disable outermost padding.
	...
	ImGuiTableFlags_HighlightHoveredColumn = 1
		<< 28, // Highlight column headers when hovered (may evolve into a
	// fuller highlight)
...
enum ImGuiTableColumnFlags_ {
	...
	ImGuiTableColumnFlags_DefaultSort = 1
		<< 2, // Default as a sorting column.
	...
	ImGuiTableColumnFlags_AngledHeader = 1
		<< 18, // TableHeadersRow() will submit an angled header row for this
...
enum ImGuiTableRowFlags_ {
	...
	ImGuiTableRowFlags_Headers = 1
		<< 0, // Identify header row (set default background color + width of
...
// available in imgui.h because some of our public structures are relying on
// it.
...
// style values, and ImGui::PushStyleColor(ImGuiCol_XXX)/PopStyleColor() for
// colors.
//-----------------------------------------------------------------------------
...
struct ImGuiStyle {
	...
	ImVec2 WindowMinSize; // Minimum window size. This is a global setting. If
		// you want to constrain inDiviseurdual windows, use
...
// structure. Access via ImGui::GetIO(). Read 'Programmer guide' section in
// .cpp file for general usage. It is generally expected that:
...
struct ImGuiIO {
	...
	ImGuiConfigFlags
		ConfigFlags; // = 0              // See ImGuiConfigFlags_
	...
	bool FontAllowUserScaling; // = false          // [OBSOLETE] Allow user
				   // scaling text of inDiviseurdual window with
				   // CTRL+Wheel.
	...
	//   or refer to ImOsIsDebuggerPresent() imgui_test_engine/imgui_te_utils.cpp
	...
	IMGUI_API void AddInputCharacterUTF16(ImWchar16
			c); // Queue a new character input from a UTF-16 character, it
			    // can be a surrogate
	IMGUI_API void AddInputCharactersUTF8(const char *
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
	IMGUI_API void
	ClearInputCharacters(); // [Obsoleted in 1.89.8] Clear the
				// current frame text input buffer. Now
	...
	ImGuiKeyData KeysData
		[ImGuiKey_NamedKey_COUNT]; // Key state for all known keys. MUST
	...
	bool MouseDoubleClicked
		[5]; // Has mouse button been double-clicked? (same as
	...
	bool MouseDownOwnedUnlessPopupClose
		[5];			// Track if button was clicked inside
					// a dear imgui window.
...
// - ImGuiInputTextFlags_CallbackEdit:        Callback on buffer edit. Note
// that InputText() already returns true on edit + you can always use
// IsItemEdited(). The callback is useful to manipulate the underlying buffer
...
struct ImGuiInputTextCallbackData {
	...
	ImWchar EventChar; // Character input                      // Read-write   //
		...
		// EventChar=0;
	ImGuiKey
		EventKey; // Key pressed (Up/Down/TAB)            // Read-only    //
			  // [Completion,History]
...
// and rare insertion (typically tied to user interactions aka max once a
// frame) You can use it as custom user storage for temporary values. Declare
...
// you can perform coarse clipping based on visibility to only submit items
// that are in view. The clipper calculates the range of visible items and
...
// - It is important that we are keeping those disabled by default so they
// don't leak in user space.
...
// - User can declare their own format by #defining the 5 _SHIFT/_MASK macros
// in their imconfig file.
...
// convenience to pass colors in either ImU32 or ImVec4 formats. Explicitly
...
enum ImGuiMultiSelectFlags_ {
	...
	ImGuiMultiSelectFlags_NoSelectAll = 1
		<< 1, // Disable CTRL+A shortcut to select all.
	ImGuiMultiSelectFlags_NoRangeSelect = 1
		<< 2, // Disable Shift+selection mouse/keyboard support (useful for
	...
	ImGuiMultiSelectFlags_NoAutoSelect = 1
		<< 3, // Disable selecting items when navigating (useful for e.g.
	...
	ImGuiMultiSelectFlags_NoAutoClearOnReselect = 1
		<< 5, // Disable clearing selection when clicking/selecting an already
		      // selected item.
	ImGuiMultiSelectFlags_BoxSelect1d = 1
		<< 6, // Enable box-selection with same width and same x pos items (e.g.
	...
	// at empty space.
	ImGuiMultiSelectFlags_BoxSelect2d = 1
	...
	ImGuiMultiSelectFlags_BoxSelectNoScroll = 1
		<< 8, // Disable scrolling when box-selecting near edges of scope.
	ImGuiMultiSelectFlags_ClearOnEscape = 1
		<< 9, // Clear selection when pressing Escape while scope is focused.
	...
	ImGuiMultiSelectFlags_ScopeWindow = 1
		<< 11, // Scope for _BoxSelect and _ClearOnClickVoid is whole window
	// (Default). Use if BeginMultiSelect() covers a whole window or
	// used a single time in same window.
	ImGuiMultiSelectFlags_ScopeRect = 1
	...
	ImGuiMultiSelectFlags_SelectOnClick = 1
		<< 13, // Apply selection on mouse down when clicking on unselected
	// item. (Default)
	ImGuiMultiSelectFlags_SelectOnClickRelease = 1
		<< 14, // Apply selection on mouse release when clicking an unselected
	...
	// does.
	ImGuiMultiSelectFlags_NavWrapX = 1
		<< 16, // [Temporary] Enable navigation wrapping on X axis. Provided as
...
// - Below: who reads/writes each fields? 'r'=read, 'w'=write,
// 'ms'=multi-select code, 'app'=application/user code.
...
// - Use this helper as a convenience. We use our simple key->value
// ImGuiStorage as a std::set<ImGuiID> replacement.
// - Use your own external storage: e.g. std::set<MyObjectId>,
// std::vector<MyObjectId>, interval trees, intrusively stored selection etc.
...
// own thing. See https://github.com/ocornut/imgui/wiki/Multi-Select for
// details and pseudo-code using this helper.
...
// ImDrawListSplitter, ImDrawListFlags, ImDrawList, ImDrawData) Hold a series
// of drawing commands. The user provides a renderer for ImDrawData which
...
// callbacks just to create your own widget or customized UI rendering, you can
// poke into the draw list for that! Draw callback may be useful for example
// to:
...
//   VtxOffset fields.
// - The ClipRect/TexRef/VtxOffset fields must be contiguous as we memcmp()
...
struct ImDrawCmd {
	...
	// function for an upcoming feature) Since 1.92: removed ImDrawCmd::TextureId
...
// pos (8 bytes), ImVec2 uv (8 bytes), ImU32 col (4 bytes), but you can
// re-order them or add other fields as needed to simplify integration in your
// engine. The type has to be described within the macro (you can either
// declare the struct or use a typedef). This is because ImVec2/ImU32 are
// likely not declared at the time you'd want to set your type up. NOTE: IMGUI
// DOESN'T CLEAR THE STRUCTURE AND DOESN'T CALL A CONSTRUCTOR SO ANY CUSTOM
// FIELD WILL BE UNINITIALIZED. IF YOU ADD EXTRA FIELDS (SUCH AS A 'Z'
...
// which can be drawn into out of order. This is used by the Columns/Tables
// API, so items of each column can be batched together in a same draw call.
struct ImDrawListSplitter {
	...
	IMGUI_API void SetCurrentChannel(
...
enum ImDrawFlags_ {
	...
	ImDrawFlags_Closed = 1
		<< 0, // PathStroke(), AddPolyline(): specify that shape should be
...
// At the end of the frame, all command lists are passed to your
// ImGuiIO::RenderDrawListFn function for rendering. Each dear imgui window
// contains its own ImDrawList. You can use ImGui::GetWindowDrawList() to
// access the current window draw list and draw custom primitives.
// You can interleave normal ImGui:: calls and adding primitives to the current
// draw list. In single viewport mode, top-left is == GetMainViewport()->Pos
// (generally 0,0), bottom-right is == GetMainViewport()->Pos+Size (generally
// io.DisplaySize). You are totally free to apply whatever transformation
// matrix you want to the data (depending on the use of the transformation you
// may want to apply it to ClipRect as well!) Important: Primitives are always
...
struct ImDrawList {
	...
	ImDrawVert
		*_VtxWritePtr; // [Internal] point within VtxBuffer.Data after
	// each add command (to avoid using the ImVector<>
	// operators too much)
	ImDrawIdx *
		_IdxWritePtr; // [Internal] point within IdxBuffer.Data after each
	...
	IMGUI_API void AddRect(const ImVec2 &p_min, const ImVec2 &p_max,
	...
	IMGUI_API void AddEllipseFilled(const ImVec2 &center,
		const ImVec2 &radius, ImU32 col, float rot = 0.0f,
		int num_segments = 0);
	...
	IMGUI_API void AddConcavePolyFilled(
		const ImVec2 *points, int num_points, ImU32 col);
	...
	IMGUI_API void AddImageQuad(ImTextureRef tex_ref, const ImVec2 &p1,
	...
	IMGUI_API void PathEllipticalArcTo(const ImVec2 &center,
		const ImVec2 &radius, float rot, float a_min, float a_max,
	...
	// multiple clipping rectangles, prefer to append into separate channels then
	// merge at the end)
	...
	IMGUI_API void _PathArcToN(const ImVec2 &center, float radius,
		float a_min, float a_max, int num_segments);
};

// All draw data to render a Dear ImGui frame
// (NB: the style and the naming convention here is a little inconsistent, we
// currently preserve them for backward compatibility purpose, as this is one
// of the oldest structure exposed by the library! Basically, ImDrawList ==
// CmdList)
struct ImDrawData {
...
// inDiviseurdual rectangles to copy to the graphics system. You may use
...
// own book-keeping. Some backends may have enough with TexID and not need
// both. In columns below: who reads/writes each fields? 'r'=read, 'w'=write,
...
struct ImTextureData {
	...
	ImTextureFormat
		Format;	   // w    r   // ImTextureFormat_RGBA32 (default) or
			   // ImTextureFormat_Alpha8
	int Width;	   // w    r   // Texture width
	int Height;	   // w    r   // Texture height
	int BytesPerPixel; // w    r   // 4 or 1
	unsigned char
		*Pixels; // w    r   // Pointer to buffer holding 'Width*Height'
			 // pixels and 'Width*Height*BytesPerPixels' bytes.
	ImTextureRect
		UsedRect; // w    r   // Bounding box encompassing all past and
			  // queued Updates[].
	ImTextureRect
		UpdateRect; // w    r   // Bounding box encompassing all queued
			    // Updates[].
	ImVector<ImTextureRect>
		Updates;  // w    r   // Array of inDiviseurdual updates.
...
// ImFontAtlas, ImFontGlyphRangesBuilder, ImFont)
//-----------------------------------------------------------------------------
...
struct ImFontConfig {
	// Data Source
	char Name
		[40]; // <auto>   // Name (strictly to ease debugging, hence limited
		      // size buffer)
	...
	// ranges from a font source, when merging fonts
	// with overlapping glyphs. Use "Input Glyphs
	// Overlap Detection Tool" to find about your
	// overlapping ranges.
	...
	const ImFontLoader
		*FontLoader; // Custom font backend for this source (other
		// use one stored in ImFontAtlas)
...
struct ImFontGlyph {
	...
	unsigned int
		Visible : 1; // Flag to indicate glyph has no visible pixels
...
struct ImFontAtlas {
	...
	IMGUI_API ImFont *AddFontFromMemoryTTF(void *font_data,
		int font_data_size, float size_pixels = 0.0f,
	...
	// those soon:
	IMGUI_API void ClearInputData(); // [OBSOLETE] Clear input data (all
		// ImFontConfig structures including sizes,
		// TTF data, glyph ranges, etc.) = all the
	...
	IMGUI_API bool
	Build(); // Build pixels data. This is called automatically
	...
	const ImFontLoader *
		FontLoader; // Font loader opaque interface (default to stb_truetype,
			    // can be changed to use FreeType by defining
			    // IMGUI_ENABLE_FREETYPE). Don't set directly!
	const char
		*FontLoaderName; // Font loader name (for display e.g. in About
				 // box) == FontLoader->Name
	...
	ImGuiContext
		*OwnerContext; // Context which own the atlas will be in charge
	...
	IMGUI_API ImFontAtlasRectId AddCustomRectFontGlyph(ImFont *font,
		ImWchar codepoint, int w, int h, float advance_x,
		const ImVec2 &offset = ImVec2(0,
			0)); // OBSOLETED in 1.92.X: Use custom ImFontLoader in ImFontConfig
	IMGUI_API ImFontAtlasRectId AddCustomRectFontGlyphForSize(ImFont *font,
		float font_size, ImWchar codepoint, int w, int h,
		float advance_x,
		const ImVec2 &offset = ImVec2(
			0, 0)); // ADDED AND OBSOLETED in 1.92.X
#endif
...
enum ImFontFlags_ {
	ImFontFlags_None = 0,
	ImFontFlags_NoLoadError = 1
		<< 1, // Disable throwing an error/assert when calling AddFontXXX() with
	...
	ImFontFlags_LockBakedSizes = 1
		<< 3, // [Internal] Disable loading new baked sizes, disable garbage
...
struct ImFont {
	...
	ImVector<ImFontConfig *>
		Sources; // 16    // in  // List of sources. Pointers
...
// Using an indirection to avoid patching ImDrawCmd after a SetTexID() call
// (but this could be an alternative solution too)
inline ImTextureID ImDrawCmd::GetTexID() const
{
	...
	if (TexRef._TexData != NULL)
		IM_ASSERT(tex_id != ImTextureID_Invalid &&
			"ImDrawCmd is referring to ImTextureData that wasn't "
			"uploaded to "
			"graphics system. Backend must call "
...
GetWindowContentRegionMin(); // Content boundaries min for the window (roughly
	// (0,0)-Scroll), in window-local coordinates.
	// You should never need this. Always use
...
static inline bool BeginChildFrame(
	ImGuiID id, const ImVec2 &size, ImGuiWindowFlags window_flags = 0)
{
...
// size, const ImVec2& uv0 = ImVec2(0, 0), const ImVec2& uv1 = ImVec2(1, 1),
// int frame_padding = -1, const ImVec4& bg_col = ImVec4(0, 0, 0, 0), const
// ImVec4& tint_col = ImVec4(1, 1, 1, 1)); // --> Use new ImageButton()
// signature (explicit item id, regular FramePadding). Refer to code in 1.91 if
// you want to grab a copy of this version.
//-- OBSOLETED in 1.88 (from May 2022)
// static inline void  CaptureKeyboardFromApp(bool want_capture_keyboard =
// true)                   {
// SetNextFrameWantCaptureKeyboard(want_capture_keyboard); } // Renamed as name
// was misleading + removed default value. static inline void
...
// When using a 1.87+ backend using io.AddKeyEvent(), calling GetKeyIndex()
// with ANY ImGuiKey_XXXX values will return the same value! static inline
// ImGuiKey GetKeyIndex(ImGuiKey key) { IM_ASSERT(key >=
...
// = -1) { float height = GetTextLineHeightWithSpacing() * ((height_in_items <
// 0 ? ImMin(items_count, 7) : height_in_items) + 0.25f) +
// GetStyle().FramePadding.y * 2.0f; return BeginListBox(label, ImVec2(0.0f,
// height)); } // Helper to calculate size from items_count and height_in_items
...
// OBSOLETED in 1.78 (from June 2020) static inline bool  DragFloat4(const
// char* label, float v[4], float v_speed, float v_min, float v_max, const
// char* format, float power = 1.0f) { return DragScalarN(label,
...
// power = 1.0f)              { return SliderScalarN(label,
...
// OBSOLETED in 1.78 (from June 2020) static inline bool  SliderFloat4(const
// char* label, float v[4], float v_min, float v_max, const char* format, float
// power = 1.0f)              { return SliderScalarN(label,
...
// 0.0f)); }                      // OBSOLETED in 1.53 (between Oct 2017 and
// Dec 2017) static inline float GetItemsLineHeightWithSpacing()       { return
...
// 2017): Equivalent of using SetNextWindowSize(size, ImGuiCond_FirstUseEver)
// and SetNextWindowBgAlpha(). static inline bool
...
// partly broken. You probably want to use the io.WantCaptureMouse flag
...
[Internal] For custom font glyphs only: glyph is colored, removed tinting.
    float           GlyphAdvanceX;  // Input    // [Internal] For custom font
glyphs only: glyph xadvance ImVec2          GlyphOffset;    // Input    //
[Internal] For custom font glyphs only: glyph display offset ImFont* Font; //
Input    // [Internal] For custom font glyphs only: target font
    ImFontAtlasCustomRect()         { X = Y = 0xFFFF; Width = Height = 0;
GlyphID = 0; GlyphColored = 0; GlyphAdvanceX = 0.0f; GlyphOffset = ImVec2(0,
0); Font = NULL; } bool IsPacked() const           { return X != 0xFFFF; }
};*/
...
//    Was == 0 prior to 1.82, this is now == ImDrawFlags_RoundCornersNone which
//    is != 0 and not implicit ImDrawCornerFlags_TopLeft   =
...
//    to 1.82. ImDrawCornerFlags_BotLeft   =
//    ImDrawFlags_RoundCornersBottomLeft,   // Was == 0x04 (1 << 2) prior
//    to 1.82. ImDrawCornerFlags_BotRight  =
//    ImDrawFlags_RoundCornersBottomRight,  // Was == 0x08 (1 << 3) prior
//    to 1.82. ImDrawCornerFlags_All       = ImDrawFlags_RoundCornersAll, //
//    Was == 0x0F prior to 1.82 ImDrawCornerFlags_Top       =
...
// in 1.88 (from April 2022). Exceptionally commented out ahead of
// obscolescence schedule to reduce confusion and because they were not meant
